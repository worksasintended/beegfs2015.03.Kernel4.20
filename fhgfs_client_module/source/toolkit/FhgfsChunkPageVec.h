...
static inline  fhgfs_bool FhgfsChunkPageVec_pushPage(FhgfsChunkPageVec* this, struct page* page,
   int usedPageLength);
...
{
   fhgfs_bool isPoolAlloc = fhgfs_false;
 ...
 *
 * Note: Returns fhgfs_false if the page-vec is full or a non-consecutive page is to be added
 */
fhgfs_bool FhgfsChunkPageVec_pushPage(FhgfsChunkPageVec* this, struct page* page,
   int usedPageLength)
{
   const char* logContext = __func__;
   fhgfs_bool pushSuccess;

   if ((this->size  + this->firstPageChunkOffset) == this->numChunkPages)
      return fhgfs_false; // pageVec full

   if ((this->size) && (this->lastPageIdx + 1 != page->index) )
      return fhgfs_false; // non-consecutive page
