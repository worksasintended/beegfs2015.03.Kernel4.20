   ...
   FhgfsCommKitVec* comm)
{
   ...
   { // given targetID refers to a buddy mirror group
      MirrorBuddyGroupMapper* mirrorBuddies = App_getMirrorBuddyGroupMapper(commHelper->app);
   ...
   // connect
   comm->sock = NodeConnPool_acquireStreamSocketEx(connPool, fhgfs_true);
   if(!comm->sock)
   {  // connection error
   ...
   sendRes = comm->sock->send(comm->sock, comm->msgBuf, comm->hdrLen, 0);
   ...
   FhgfsCommKitVec* comm)
{
   ...
   if(likely(comm->node) )
      NodeStoreEx_releaseNode(storageNodes, &comm->node);
   ...
   FhgfsCommKitVec* comm)
{
   ...
   fhgfs_bool needInodeRefresh = fhgfs_true;
   FhgfsPage* fhgfsPage;
   size_t pgCount = 0;
   ...
   while (1)
   {
      ...
      if (pageReadRes >= 0)
      {
         ...
         needInodeRefresh = fhgfs_false; // a single inode refresh is sufficient
   ...
   FhgfsCommKitVec* comm)
{
   ...
   sendRes = comm->sock->send(
      comm->sock, comm->msgBuf, comm->hdrLen, 0);
   ...
   FhgfsCommKitVec* comm)
{
   ...
   while (fhgfs_true)
   {
      ...
      sendDataPartRes = comm->sock->send(comm->sock, data, dataLength, 0);
   ...
   FhgfsCommKitVec* comm)
{
   ...
   { // got response => deserialize it
      ...
      { // response invalid
         ...
         WriteLocalFileRespMsg_uninit( (NetMessage*)&comm->write.respMsg);
   ...
   FhgfsCommKitVec* comm, Fhgfs_RWType rwType)
{
   ...
   fhgfs_bool isSignalPending = fhgfs_false;
   const char* logCommInterruptedTxt = "Communication interrupted by signal";
