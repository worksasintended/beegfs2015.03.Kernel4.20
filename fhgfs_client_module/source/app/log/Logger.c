...
void Logger_init(Logger* this, App* app, Config* cfg)
{
   ...
   this->loggerdReachable = fhgfs_true;

   this->outputMutex = Mutex_construct();
   ...
   this->multiLockMutex = Mutex_construct();
}
...
void Logger_uninit(Logger* this)
{
   ...
   Mutex_destruct(this->multiLockMutex);
   Mutex_destruct(this->outputMutex);
}
   ...
   const char* context, const char* msg)
{
   ...
   SignalTk_blockSignals(fhgfs_false, &oldSignalSet); // B L O C K _ S I G s
...
cleanup_request:
   LogMsg_uninit( (NetMessage*)&requestMsg);
   ...
   const char* context, const char* msgFormat, va_list args)
{
   ...
   __Logger_setCurrentOutputPID(this, os_getCurrentPID() ); // grab currentOutputPID


   // evaluate msgFormat
   os_vsnprintf(this->logFormattedBuf, LOGGER_LOGBUF_SIZE, msgFormat, args);

   // extend context
   if(this->clientID)
      os_snprintf(this->logContextBuf, LOGGER_LOGBUF_SIZE, "%s: %s", this->clientID, context);
   else
      os_snprintf(this->logContextBuf, LOGGER_LOGBUF_SIZE, "%s", context);
 ...
 *
 * @return fhgfs_true if the currentOutputPID is set to the current thread and logging cannot continue;
 */
fhgfs_bool __Logger_checkThreadMultiLock(Logger* this)
{
   fhgfs_bool retVal = fhgfs_false;

   Mutex_lock(this->multiLockMutex);

   if(this->currentOutputPID == os_getCurrentPID() )
...
void __Logger_logUnreachableLoggerd(Logger* this)
{
   ...
   else
   {
      ...
      this->loggerdReachable = fhgfs_false;
   }
}
 ...
 *
 * @return fhgfs_false if string didn't match any known log topic.
 */
fhgfs_bool Logger_getLogTopicFromStr(const char* logTopicStr, LogTopic* outLogTopic)
{
   int i;
   ...
   for(i=0; i < LogTopic_LAST; i++)
   {
      ...
      if(!os_strnicmp(logTopicStr, currentLogTopicStr, os_strlen(currentLogTopicStr) ) )
      {
         *outLogTopic = i;
         return fhgfs_true;
      }
