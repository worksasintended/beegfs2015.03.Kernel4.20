...
void AckManager_init(AckManager* this, App* app)
{
   ...
   Thread_init( (Thread*)this, BEEGFS_THREAD_NAME_PREFIX_STR "AckMgr", __AckManager_run);
   ...
   this->ackMsgBuf = os_vmalloc(ACKMANAGER_MSGBUF_LEN);

   this->ackQueueMutex = Mutex_construct();
   this->ackQueueAddedCond = Condition_construct();
   this->ackQueue = PointerList_construct();
}
...
void AckManager_uninit(AckManager* this)
{
   ...
   PointerList_destruct(this->ackQueue);
   Condition_destruct(this->ackQueueAddedCond);
   Mutex_destruct(this->ackQueueMutex);

   os_vfree(this->ackMsgBuf);
...
void __AckManager_processAckQueue(AckManager* this)
{
   ...
   while(!PointerListIter_end(&iter) && !Thread_getSelfTerminate( (Thread*)this) )
   {
      ...
      AckMsgEx msg;
      unsigned msgLen;
      fhgfs_bool serialRes;
      NodeConnPool* connPool;
      int currentRetryNum;
      Socket* sock;
      ssize_t sendRes = 0;
      fhgfs_bool removeAllNextByNode = fhgfs_false;
      ...
      for(currentRetryNum=0; currentRetryNum <= maxNumCommRetries; currentRetryNum++)
      {
         ...
         if(likely(sock) )
         { // send msg
            sendRes = sock->send(sock, this->ackMsgBuf, msgLen, 0);
         ...
         { // no connection or communication error
            ...
            removeAllNextByNode = fhgfs_true; // (only effective if no more retries)

            continue;
         }

         removeAllNextByNode = fhgfs_false;
 ...
 */
void __AckManager_removeQueueEntriesByNode(AckManager* this, uint16_t nodeID,
   PointerListIter iter)
{
   while(!PointerListIter_end(&iter) )
   {
      ...
      if(nodeID == currentAck->metaNodeID)
      { // nodeID matches
         __AckManager_freeQueueEntry(this, currentAck);
 ...
 */
void AckManager_addAckToQueue(AckManager* this, uint16_t metaNodeID, const char* ackID)
{
