   ...
   unsigned short streamPort, NicAddressList* nicList)
{
   ...
   this->nicList = ListTk_cloneNicAddressList(nicList);
   ...
   os_memset(&this->localNicCaps, 0, sizeof(this->localNicCaps) );
   os_memset(&this->stats, 0, sizeof(this->stats) );
   os_memset(&this->errState, 0, sizeof(this->errState) );

   this->logConnErrors = fhgfs_true;
}
...
void NodeConnPool_uninit(NodeConnPool* this)
{
   ...
   while(ConnectionList_length(&this->connList) )
   {
      ...
      ConnectionListIter_uninit(&connIter);
   }
   ...
   // normal clean-up
   NicAddressList_destruct(this->nicList);
...
{
   return NodeConnPool_acquireStreamSocketEx(this, fhgfs_true);
}

/**
 * note: this initially checks for pending signals and returns NULL immediately if signal pending.
 *
 * @param fhgfs_true to allow waiting if all avaiable conns are currently in use; if waiting is not
 ...
 * @return connected socket; NULL on error or pending signal
 */
Socket* NodeConnPool_acquireStreamSocketEx(NodeConnPool* this, fhgfs_bool allowWaiting)
{
   ...
   fhgfs_bool isPrimaryInterface = fhgfs_true; // used to set expiration for non-primary interfaces;
   ...
   { // wait for a conn to become available (or disconnected)
      ...
      { // sleep interruptbile
         cond_wait_res_t waitRes = Condition_waitInterruptible(this->changeCond, this->mutex);
   ...
   { // established connection available => grab it
      ...
      PooledSocket_setAvailable(pooledSock, fhgfs_false);
   ...
   nicListCopy = ListTk_cloneNicAddressList(this->nicList);

   if(unlikely(!nicListCopy) )
   { // memory allocation for nic list copy failed
      Mutex_unlock(this->mutex); // U N L O C K
      return NULL;
   }
   ...
   for( ; !NicAddressListIter_end(&nicIter); NicAddressListIter_next(&nicIter) )
   {
      ...
      if(!Socket_getSockValid( (Socket*)sock) )
      ...
      connectRes = sock->connectByIP(sock, &nicAddr->ipAddr, port);

      if(connectRes)
      { // connected
         uint32_t peerIP = Socket_getPeerIP(sock);
         ...
         {
            PooledSocket_setExpireTimeStart( (PooledSocket*)sock);
         }

         os_kfree(endpointStr);
      ...
      { // not connected
         ...
         isPrimaryInterface = fhgfs_false;
   ...
   NicAddressList_destruct(nicListCopy);
...
void NodeConnPool_releaseStreamSocket(NodeConnPool* this, Socket* sock)
{
   ...
   PooledSocket_setAvailable(pooledSock, fhgfs_true);
...
void NodeConnPool_invalidateStreamSocket(NodeConnPool* this, Socket* sock)
{
   ...
   numInvalidated = __NodeConnPool_invalidateAvailableStreams(this, fhgfs_false);
...
void __NodeConnPool_invalidateSpecificStreamSocket(NodeConnPool* this, Socket* sock)
{
   ...
   fhgfs_bool sockValid = fhgfs_true;
   ConnectionListIter connIter;
   PooledSocket* pooledSock = (PooledSocket*)sock;
   fhgfs_bool shutdownRes;
   ...
   shutdownRes = sock->shutdownAndRecvDisconnect(sock, NODECONNPOOL_SHUTDOWN_WAITTIMEMS);
   if(shutdownRes)
   {
 ...
 */
unsigned __NodeConnPool_invalidateAvailableStreams(NodeConnPool* this, fhgfs_bool idleStreamsOnly)
{
   ...
   for( ; !ConnectionListIter_end(&connIter); ConnectionListIter_next(&connIter) )
   {
      ...
      PooledSocket_setAvailable(sock, fhgfs_false);
      this->availableConns--;
   ...
   ConnectionListIter_uninit(&availableIter);
...
unsigned NodeConnPool_disconnectAndResetIdleStreams(NodeConnPool* this)
{
   ...
   numInvalidated = __NodeConnPool_invalidateAvailableStreams(this, fhgfs_true);
...
fhgfs_bool __NodeConnPool_applySocketOptionsPreConnect(NodeConnPool* this, Socket* sock)
{
 ...
 */
fhgfs_bool __NodeConnPool_applySocketOptionsConnected(NodeConnPool* this, Socket* sock)
{
   ...
   fhgfs_bool allSuccessful = fhgfs_true;
       ...
       (sockType == NICADDRTYPE_SDP) )
   {
      ...
      fhgfs_bool corkRes;
      fhgfs_bool noDelayRes;
      fhgfs_bool keepAliveRes;

      corkRes = StandardSocket_setTcpCork(standardSock, fhgfs_false);
      if(!corkRes)
      {
         Logger_log(log, Log_NOTICE, logContext, "Failed to disable TcpCork");
         allSuccessful = fhgfs_false;
      }

      noDelayRes = StandardSocket_setTcpNoDelay(standardSock, fhgfs_true);
      if(!noDelayRes)
      {
      ...
      keepAliveRes = StandardSocket_setSoKeepAlive(standardSock, fhgfs_true);
      if(!keepAliveRes)
      {
   ...
   { // authenticate channel
      ...
      sendRes = sock->send(sock, sendBuf, sendBufLen, 0);
      if(sendRes <= 0)
      {
      ...
      os_kfree(sendBuf);
      AuthenticateChannelMsg_uninit( (NetMessage*)&authMsg);
   }
   ...
   { // make channel indirect
      ...
      os_kfree(sendBuf);
      SetChannelDirectMsg_uninit( (NetMessage*)&directMsg);
   }
...
unsigned NodeConnPool_getFirstPeerName(NodeConnPool* this, NicAddrType_t nicType, ssize_t outBufLen,
   char* outBuf, fhgfs_bool* outIsNonPrimary)
{
   unsigned numWritten = 0; // return value
   ConnectionListIter connIter;
   fhgfs_bool foundMatch = fhgfs_false;
   ...
   for( ; !ConnectionListIter_end(&connIter); ConnectionListIter_next(&connIter) )
   {
      ...
      { // found a match => print to buf and stop
         numWritten += os_scnprintf(outBuf, outBufLen, "%s", Socket_getPeername(sock) );

         *outIsNonPrimary = PooledSocket_getHasExpirationTimer(pooledSock);

         foundMatch = fhgfs_true;
         break;
      }
   }
   ...
   if(!foundMatch)
   { // print "n/a"
      numWritten += os_scnprintf(outBuf, outBufLen, "busy");

      *outIsNonPrimary = fhgfs_false;
   }
 ...
 * @return fhgfs_true if port has changed
 */
fhgfs_bool NodeConnPool_updateInterfaces(NodeConnPool* this, unsigned short streamPort,
   NicAddressList* nicList)
{
   Logger* log = App_getLogger(this->app);
   const char* logContext = "NodeConn (update stream port)";

   fhgfs_bool portHasChanged = fhgfs_false; // retVal
   ...
   {
      this->streamPort = streamPort;
      portHasChanged = fhgfs_true;
   }

   { // update nicList (if allocation of new list fails, we just keep the old list)
      NicAddressList* newNicList = ListTk_cloneNicAddressList(nicList);

      if(likely(newNicList) )
      { // cloning succeeded => free old list and assign new list
         ListTk_kfreeNicAddressListElems(this->nicList);
         NicAddressList_destruct(this->nicList);
   ...
   if(portHasChanged)
   {
      unsigned numInvalidated = __NodeConnPool_invalidateAvailableStreams(this, fhgfs_false);
      if(numInvalidated)
      {
...
{
   this->errState.lastSuccessPeerIP = 0;
   this->errState.wasLastTimeCompleteFail = fhgfs_true;
}

void __NodeConnPool_setConnSuccess(NodeConnPool* this, uint32_t lastSuccessPeerIP,
   NicAddrType_t lastSuccessNicType)
{
   ...
   this->errState.wasLastTimeCompleteFail = fhgfs_false;
}

fhgfs_bool __NodeConnPool_equalsLastSuccess(NodeConnPool* this, uint32_t lastSuccessPeerIP,
   NicAddrType_t lastSuccessNicType)
{
   return (this->errState.lastSuccessPeerIP == lastSuccessPeerIP) &&
      (this->errState.lastSuccessNicType == lastSuccessNicType);

}

fhgfs_bool __NodeConnPool_isLastSuccessInitialized(NodeConnPool* this)
{
   return (this->errState.lastSuccessPeerIP != 0);
}

fhgfs_bool __NodeConnPool_shouldPrintConnectedLogMsg(NodeConnPool* this, uint32_t currentPeerIP,
...
fhgfs_bool __NodeConnPool_shouldPrintConnectFailedLogMsg(NodeConnPool* this, uint32_t currentPeerIP,
   NicAddrType_t currentNicType)
{
