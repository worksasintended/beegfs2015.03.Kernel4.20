...
#define SAFE_VFREE(p) \
   do{ if(p) {os_vfree(p); (p)=NULL;} } while(0)

#define SAFE_VFREE_NOSET(p) \
   do{ if(p) os_vfree(p); } while(0)

#define SAFE_KFREE(p) \
   do{ if(p) {os_kfree(p); (p)=NULL;} } while(0)

#define SAFE_KFREE_NOSET(p) \
   do{ if(p) os_kfree(p); } while(0)
...
// simply way to convert an enum to a string
#define ENUMSTR(ENUM) #ENUM
...
#define printk_fhgfs(levelStr, fmtStr, ...) \
   os_printk(levelStr BEEGFS_MODULE_NAME_STR ": " "%s(%u): " fmtStr, os_getCurrentCommNameStr(), \
      (unsigned)os_getCurrentPID(), ## __VA_ARGS__)
...
// __copy_to/from_user() are declared with warnings for ignored return values. this macro mutes the
// warnings (because we check up front whether everything is correct).
#define IGNORE_RETURN_VALUE(a)      IGNORE_UNUSED_VARIABLE(a)
...
// not defined for old kernel versions
