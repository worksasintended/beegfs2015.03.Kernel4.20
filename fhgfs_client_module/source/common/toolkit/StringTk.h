...
extern fhgfs_bool StringTk_strToBool(const char* s);
...
static inline char* StringTk_doubleToStr(double a);

// inliners
static inline fhgfs_bool StringTk_hasLength(const char* s);



fhgfs_bool StringTk_hasLength(const char* s)
{
   return s[0] ? fhgfs_true : fhgfs_false;
}
...
char* StringTk_doubleToStr(double a)
{
   char aStr[32];
   sprintf(aStr, "%0.3f", a);
