...
extern FhgfsOpsErr MessagingTk_requestResponseRetry(App* app, Node* node,
   NetMessage* requestMsg, unsigned respMsgType, char** outRespBuf, NetMessage** outRespMsg);
extern FhgfsOpsErr MessagingTk_requestResponseRetryAutoIntr(App* app, Node* node,
   NetMessage* requestMsg, unsigned respMsgType, char** outRespBuf, NetMessage** outRespMsg);
...
extern FhgfsOpsErr MessagingTk_requestResponseNodeRetryAutoIntr(App* app,
   RequestResponseNode* rrNode, RequestResponseArgs* rrArgs);
extern FhgfsOpsErr MessagingTk_requestResponseTargetRetryAutoIntr(App* app,
...
extern FhgfsOpsErr __MessagingTk_requestResponseNodeRetry(App* app,
   RequestResponseNode* rrNode, RequestResponseArgs* rrArgs);
extern FhgfsOpsErr __MessagingTk_requestResponseTargetRetry(App* app,
   RequestResponseTarget* rrTarget, RequestResponseArgs* rrArgs);
...
static inline ssize_t MessagingTk_recvMsgBufEx(App* app, Socket* sock, char* bufIn, size_t bufInLen,
   fhgfs_bool useDelayedInterrupt);
...
/**
 * Receive a complete message into given buffer.
 */
ssize_t MessagingTk_recvMsgBuf(App* app, Socket* sock, char* bufIn, size_t bufInLen)
{
   return MessagingTk_recvMsgBufEx(app, sock, bufIn, bufInLen, fhgfs_false);
}
 ...
 * @param useDelayedInterrupt true to enable interrupt signal after a short timeout; caller must
 * ensure that the user process actually had SIGINT enabled before setting this to true.
 * @return positive message length on success, <=0 on error (e.g. -ETIMEDOUT on recv timeout,
 * -EMSGSIZE if msg too large for buffer).
 */
ssize_t MessagingTk_recvMsgBufEx(App* app, Socket* sock, char* bufIn, size_t bufInLen,
   fhgfs_bool useDelayedInterrupt)
{
   const char* logContext = "MessagingTk (recv msg)";
   Config* config = App_getConfig(app);

   const int recvNonIntrTimeoutMS = Config_getConnRecvNonIntrTimeoutMS(config);
   const int recvNormalTimeoutMS = CONN_LONG_TIMEOUT - recvNonIntrTimeoutMS; // interruptible wait

   int recvTimeoutMS = useDelayedInterrupt ? recvNonIntrTimeoutMS : recvNormalTimeoutMS;
   ...
   recvRes = Socket_recvExactTEx(sock, bufIn, NETMSG_MIN_LENGTH, 0, recvTimeoutMS, &numReceived);

   if(unlikely(recvRes == -ETIMEDOUT) && (recvTimeoutMS == recvNonIntrTimeoutMS) )
   { // non-interruptible timeout expired => enable interrupt signal and try again
      recvTimeoutMS = recvNormalTimeoutMS;

      SignalTk_enableSigInt();

      recvRes = Socket_recvExactTEx(sock, &bufIn[numReceived], NETMSG_MIN_LENGTH, 0, recvTimeoutMS,
         &numReceived);
   }
   ...
   recvRes = Socket_recvExactTEx(sock, &bufIn[numReceived], msgLength-numReceived, 0, recvTimeoutMS,
      &numReceived);

   if(unlikely(recvRes == -ETIMEDOUT) && (recvTimeoutMS == recvNonIntrTimeoutMS) )
   { // non-interruptible timeout expired => enable interrut signal and try again
      recvTimeoutMS = recvNormalTimeoutMS;
