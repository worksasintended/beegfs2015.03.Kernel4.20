...
struct FhgfsIsizeHints
{
   fhgfs_bool ignoreIsize; // caller knows that the i_size needs to be ignored
...
extern FhgfsOpsErr FhgfsInode_referenceHandle(FhgfsInode* this, int openFlags,
   fhgfs_bool allowRWHandle, LookupIntentInfoOut* lookupInfo, FileHandleType* outHandleType);
extern FhgfsOpsErr FhgfsInode_releaseHandle(FhgfsInode* this, FileHandleType handleType);
extern fhgfs_bool FhgfsInode_hasWriteHandle(FhgfsInode* this);
...
extern fhgfs_bool FhgfsInode_removeRangeLockPID(FhgfsInode* this, int pid);
...
extern fhgfs_bool FhgfsInode_isCacheValid(FhgfsInode* this, umode_t i_mode, Config* cfg);


// private extern

extern FileHandleType __FhgfsInode_openFlagsToHandleType(int openFlags);
extern FhgfsOpsErr __FhgfsInode_referenceTrunc(FhgfsInode* this, App* app, int openFlags);
...
static inline EntryInfo* FhgfsInode_getEntryInfo(FhgfsInode* this);
...
static inline fhgfs_bool FhgfsInode_getIsFileOpen(FhgfsInode* this);
static inline fhgfs_bool FhgfsInode_getIsFileOpenByMultipleReaders(FhgfsInode* this);
...
static inline fhgfs_bool FhgfsInode_compareEntryID(FhgfsInode* this, const char* otherEntryID);
...
static inline fhgfs_bool FhgfsInode_getHasDirtyPages(FhgfsInode* this);
...
struct FhgfsInodeFileHandle
{
   ...
   fhgfs_bool needsAppendLockCleanup; // true if append lock release failed
...
struct FhgfsInode
{
   ...
   uint16_t parentNodeID; /* nodeID of the parent directory, should be moved to EntryInfo
 ...
 *
 * @return fhgfs_true if the file is currently open.
 */
fhgfs_bool FhgfsInode_getIsFileOpen(FhgfsInode* this)
{
 ...
 *
 * @return fhgfs_true if the file is currently open by more than one reader (including read+write).
 */
fhgfs_bool FhgfsInode_getIsFileOpenByMultipleReaders(FhgfsInode* this)
{
 ...
 * unlock afterwards).
 */
EntryInfo* FhgfsInode_getEntryInfo(FhgfsInode* this)
{
...
void _FhgfsInode_initRootEntryInfo(FhgfsInode* this)
{
   ...
   fhgfs_bool hasRootInfo = FhgfsOps_getHasRootEntryInfo(sb);
 ...
 *
 * @return fhgfs_true if given ID matches, fhgfs_false otherwise
 */
fhgfs_bool FhgfsInode_compareEntryID(FhgfsInode* this, const char* otherEntryID)
{
   ...
   const char* newName)
{
   ...
   {  // only update the ownerNodeID if it is not a directory
      EntryInfo_setOwnerNodeID(&this->entryInfo, newParentInfo->ownerNodeID);
   }
}
 ...
 * @return fhgfs_true if the file is currently open.
 */
fhgfs_bool FhgfsInode_getHasDirtyPages(FhgfsInode* this)
{
   return !!FhgfsInode_getNumDirtyPages(this);
}

void FhgfsInode_setParentNodeID(FhgfsInode* this, uint16_t parentNodeID)
{
...
void FhgfsInode_initIsizeHints(FhgfsInode* this, FhgfsIsizeHints* outISizeHints)
{
   ...
   outISizeHints->ignoreIsize = fhgfs_false;
   ...
   if (this)
   {
      ...
      if ((FhgfsInode_getWriteBackCounter(this) || FhgfsInode_getHasDirtyPages(this)   ||
          ...
          (mapping_writably_mapped(mapping) ) ) )
      {
         outISizeHints->ignoreIsize = fhgfs_true;
      }
   }
}
