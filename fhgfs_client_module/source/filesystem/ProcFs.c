...
int __ProcFs_readV2_metaTargetStates(struct seq_file* file, void* p)
{
   ...
   return ProcFsHelper_readV2_targetStates(file, app, metaStates, nodes, fhgfs_true);
}
   ...
   void* data)
{
   return ProcFsHelper_read_targetStates(buf, start, offset, size, eof, (App*)data,
      App_getMetaStateStore( (App*)data), App_getMetaNodes( (App*)data), fhgfs_true);
}
...
int __ProcFs_readV2_storageTargetStates(struct seq_file* file, void* p)
{
   ...
   return ProcFsHelper_readV2_targetStates(file, app, targetStates, nodes, fhgfs_false);
}
...
{
   return ProcFsHelper_read_targetStates(buf, start, offset, size, eof, (App*)data,
      App_getTargetStateStore( (App*)data), App_getStorageNodes( (App*)data), fhgfs_false);
}
