   ...
   FsDirInfo* dirInfo, fhgfs_bool forceUpdate)
{
   ...
   LOG_DEBUG_FORMATTED(log, Log_SPAM, logContext, "called. EntryID: %s, Owner: %hu",
      entryInfo->entryID, entryInfo->ownerNodeID);
 ...
 *
 * @param discardCacheOnError fhgfs_true to discard a write cache if the remote write
 * was not successful; fhgfs_false to just release it to the store in this case
 */
   ...
   loff_t offset, FhgfsInode* fhgfsInode, FsFileInfo* fileInfo, RemotingIOInfo* ioInfo)
{
   ...
   ssize_t retVal;
   fhgfs_bool isAppendWrite = (offset == -1); // true if this is an append write
   CacheBuffer* cacheBuffer;
   enum FileBufferType cacheType;
   fhgfs_bool isAppendCacheBuf; // true if the current cache buffer is an append write buffer
   ...
   { // offset doesn't fit or not enough cache room left => flush cache
      ...
      flushRes = __FhgfsOpsHelper_flushCacheUnlocked(app, fhgfsInode, fhgfs_false);
   ...
   userBufCopyRes = __os_copy_from_user(&(cacheBuffer->buf)[cacheBuffer->bufUsageLen], buf, size);
   ...
   { // cache buf used up => flush it
      FhgfsOpsErr flushRes = __FhgfsOpsHelper_flushCacheUnlocked(app, fhgfsInode, fhgfs_false);
...
exclusive_lock_path:
   ...
   userBufCopyRes = __os_copy_to_user(
   ...
   size_t size, loff_t offset, FhgfsInode* fhgfsInode, FsFileInfo* fileInfo, RemotingIOInfo* ioInfo)
{
   ...
   userBufCopyRes = __os_copy_from_user(cacheBuffer->buf, buf, size);
   if(unlikely(userBufCopyRes) )
   { // copy failed
      Logger* log = App_getLogger(app);
      Logger_log(log, Log_DEBUG, __func__, "Buffer copy from userspace failed (invalid buffer)");
   ...
   FhgfsInode* fhgfsInode, FsFileInfo* fileInfo, RemotingIOInfo* ioInfo)
{
   ...
   userBufCopyRes = __os_copy_to_user(buf, cacheBuffer->buf, MIN(size, (size_t)readRes) );
   if(unlikely(userBufCopyRes) )
   { // copy failed
      Logger* log = App_getLogger(app);
      Logger_log(log, Log_DEBUG, __func__, "Buffer copy to userspace failed (invalid buffer)");
   ...
   RemotingIOInfo* ioInfo, loff_t* outNewOffset)
{
   ...
   lockRes = FhgfsOpsRemoting_flockAppend(FhgfsInode_getEntryInfo(fhgfsInode), ioInfo, 0,
      os_getCurrentPID(), ENTRYLOCKTYPE_EXCLUSIVE);

   FhgfsInode_entryInfoReadUnlock(fhgfsInode); // UNLOCK EntryInfo

   if(unlikely(lockRes != FhgfsOpsErr_SUCCESS) )
   {
      LOG_DEBUG_FORMATTED(log, Log_DEBUG, logContext, "Append lock error: %s",
         FhgfsOpsErr_toErrString(lockRes) );
      IGNORE_UNUSED_VARIABLE(log);
      IGNORE_UNUSED_VARIABLE(logContext);

      SAFE_ASSIGN(ioInfo->needsAppendLockCleanup, fhgfs_true);
      return -lockRes;
   }
...
unlock_and_exit:
   ...
   // release MDS append lock...

   FhgfsInode_entryInfoReadLock(fhgfsInode); // LOCK EntryInfo

   unlockRes = FhgfsOpsRemoting_flockAppend(FhgfsInode_getEntryInfo(fhgfsInode), ioInfo, 0,
      os_getCurrentPID(), ENTRYLOCKTYPE_UNLOCK);
   ...
   loff_t offset, FsFileInfo* fileInfo, RemotingIOInfo* ioInfo)
{
   ...
   while(toBeRead)
   {
      ...
      { // zero-fill the remainder
         __os_clear_user(&buf[currentBufOffset+currentReadRes], currentReadSize-currentReadRes);
      }
   ...
   char** outPathRelativeToStr)
{
   ...
   Path_setAbsolute(&pathTo, fhgfs_false);
...
{
   //Logger* log = App_getLogger(app);
   //const char* logContext = "FhgfsOpsHelper_symlink";

   int retVal = 0;

   size_t toStrLen = os_strlen(to);
   FhgfsOpsErr mkRes;
   ...
   openRes = FhgfsOpsRemoting_openfile(outEntryInfo, &ioInfo);
   if(openRes != FhgfsOpsErr_SUCCESS)
   { // error
      FhgfsOpsRemoting_unlinkfile(app, parentInfo, createInfo->entryName);
   ...
   { // error
      FhgfsOpsHelper_closefileWithAsyncRetry(outEntryInfo, &ioInfo);
      FhgfsOpsRemoting_unlinkfile(app, parentInfo, createInfo->entryName);
   ...
   char __user *buf, size_t size, loff_t offset)
{
   ...
   openRes = FhgfsOpsRemoting_openfile(entryInfo, &ioInfo);
   ...
   closeRes = FhgfsOpsHelper_closefileWithAsyncRetry(entryInfo, &ioInfo);
   ...
   size_t size, loff_t offset)
{
   ...
   referenceRes = FhgfsInode_referenceHandle(fhgfsInode, OPENFILE_ACCESS_WRITE, fhgfs_true, NULL,
      &handleType);
   ...
   { // error
      FhgfsInode_releaseHandle(fhgfsInode, handleType);
   ...
   releaseRes = FhgfsInode_releaseHandle(fhgfsInode, handleType);
