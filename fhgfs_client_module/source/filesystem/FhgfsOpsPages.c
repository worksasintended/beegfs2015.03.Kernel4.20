...
static int _FhgfsOpsPages_sendPageVec(FhgfsPageData* pageData,
   struct inode* inode, fhgfs_bool isFinal, Fhgfs_RWType rwType);
 ...
 */
fhgfs_bool FhgfsOpsPages_initPageListVecCache(void)
{
...
void __FhgfsOpsPages_incInodeFileSizeOnPagedRead(struct inode* inode, loff_t offset, size_t readRes)
{
    ...
    */
   __FhgfsOps_doRefreshInode(app, inode, NULL, &iSizeHints, fhgfs_true);
   ...
   {  // _refreshInode was not sufficient, force a meta-update
      ...
      if (refreshRes != FhgfsOpsErr_SUCCESS)
      {
         ...
         // TODO: Return an io-error to the application to notify it about a too short read?
      }
 ...
 */
int _FhgfsOpsPages_sendPageVec(FhgfsPageData* pageData, struct inode* inode, fhgfs_bool isFinal,
   Fhgfs_RWType rwType)
{
   ...
   int retVal = 0;
   fhgfs_bool queueSuccess;

   #if 0
      Logger_logFormattedWithEntryID(pageData->inode, Log_SPAM, logContext,
         "from-idx: %ld to-idx: %ld",
         pageData->chunkPageVec->firstPageIdx, pageData->chunkPageVec->lastPageIdx);
   #endif
   ...
   if (!isFinal)
   {
      ...
      {
         retVal = -ENOMEM;
         // printk(KERN_INFO "Allocating new pageVec failed!!!\n");
      }
   }
...
FhgfsOpsErr _FhgfsOpsPages_referenceFileHandle(FhgfsPageData* writePageData, unsigned openFlags)
{
   ...
   {  // success
      ...
      writePageData->isReferenced = fhgfs_true;
   }
 ...
 * (chunk is full) a write request will be send immediately (TODO: async through queue).
 *
 * @return 0 on success, negative linux error code on error
 */
int FhgfsOpsPages_writePageCallBack(struct page *page, struct writeback_control *wbc, void *dataPtr)
{
   ...
   const fhgfs_bool finalWrite = fhgfs_false; /* When called from this callBack method, finalWrite
                                               * is always false. */

   fhgfs_bool pageVecWasSent = fhgfs_false;

   int referenceRes = _FhgfsOpsPages_referenceWriteFileHandle(writePageData);

   #if 0
      Logger_logFormattedWithEntryID(inode, Log_NOTICE, logContext, "ino: %llu; index: %zd",
   ...
   while(1) // repeats only once, until pageVecWasSent == true
   {
      ...
      #if 0
         Logger_logFormattedWithEntryID(inode, Log_ERR, logContext,
            "pageVec push res: %d, page-index: %ld", pushSucces, page->index);
      #endif
      ...
      {  // push success
         #if 0
            if (writePageData->chunkPageVec->size == 1)
               Logger_logFormattedWithEntryID(inode, Log_SPAM, logContext,
                  "Started new pageVec, index: %ld", writePageData->chunkPageVec->firstPageIdx);
         #endif
      }
   ...
   struct page* page)
{
   ...
   if (pageData.chunkPageVec)
   {
      ...
      writeRes = _FhgfsOpsPages_sendPageVec(&pageData, inode, fhgfs_true, BEEGFS_RWTYPE_WRITE);
...
#endif // KERNEL_HAS_NO_WRITE_CACHE_PAGES
 ...
 *
 * */
fhgfs_bool FhgfsOpsPages_isShortRead(struct inode* inode, pgoff_t pageIndex,
   fhgfs_bool needInodeRefresh)
{
   ...
   // printk(KERN_INFO "%s: need-refresh: %d pageIndex: %ld fileEndIndex: %ld\n",
   //   __func__, (int) needInodeRefresh, pageIndex, fileEndIndex);
...
int FhgfsOpsPages_readPageCallBack(void *dataPtr, struct page *page)
{
   ...
   const fhgfs_bool finalRead = fhgfs_false; // is not the final read from this function
   ...
   while(1) // repeats only once, until pageVecWasSent == true
   {
      ...
      {  // push success
         #if 0
            if (pageData->chunkPageVec->size == 1)
               Logger_logFormattedWithEntryID(inode, Log_SPAM, logContext,
                  "Started new pageVec, index: %ld", pageData->chunkPageVec->firstPageIdx);
         #endif
      }
   ...
   struct list_head* pageList, struct page* page)
{
   ...
   if (pageData.chunkPageVec)
   {
      ...
      readRes = _FhgfsOpsPages_sendPageVec(&pageData, inode, fhgfs_true, BEEGFS_RWTYPE_READ);
