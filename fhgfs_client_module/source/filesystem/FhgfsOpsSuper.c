...
static struct super_operations fhgfs_super_ops =
{
   ...
   .drop_inode    = os_GenericDropInode,
...
{
   //printk_fhgfs(KERN_INFO, "Stopping App...\n"); // debug in

   App_stop(app);

   //printk_fhgfs(KERN_INFO, "Cleaning up...\n"); // debug in
   ...
   //printk_fhgfs(KERN_INFO, "App destructed.\n"); // debug in
}
...
int __FhgfsOps_constructFsInfo(struct super_block* sb, void* rawMountOptions)
{
   ...
   FhgfsOps_setHasRootEntryInfo(sb, fhgfs_false);
   FhgfsOps_setIsRootInited(sb, fhgfs_false);
...
int FhgfsOps_fillSuper(struct super_block* sb, void* rawMountOptions, int silent)
{
   ...
   EntryInfo_init(&entryInfo, 0, StringTk_strDup(""), StringTk_strDup(""),
...
void FhgfsOps_killSB(struct super_block* sb)
{
   ...
   if (app) // might be NULL on unsuccessful mount attempt
      App_setConnRetriesEnabled(app, fhgfs_false); // faster umount on communication errors
