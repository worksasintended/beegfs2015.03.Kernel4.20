...
loff_t FhgfsOps_llseekdir(struct file *file, loff_t offset, int origin)
{
   ...
   if(likely(retVal >= 0) )
   {
      ...
      FsDirInfo_setEndOfDir(dirInfo, fhgfs_false);
   }
...
#endif // LINUX_VERSION_CODE
{
   ...
   for( ; ; )
   {
      ...
      refreshRes = FhgfsOpsHelper_refreshDirInfoIncremental(app,
         FhgfsInode_getEntryInfo(fhgfsInode), dirInfo, fhgfs_false);
...
int FhgfsOps_openReferenceHandle(App* app, struct inode* inode, struct file* file,
   unsigned openFlags, LookupIntentInfoOut* lookupInfo)
{
   Config* cfg = App_getConfig(app);
   Logger* log = App_getLogger(app);
   ...
   openRes = FhgfsInode_referenceHandle(fhgfsInode, fhgfsOpenFlags, fhgfs_false, lookupInfo,
      &handleType);
   ...
   { // success => file is open (=> handle open flags)
      ...
      if(file->f_flags & O_APPEND)
         FsFileInfo_setAppending(fileInfo, fhgfs_true);
      ...
      { // disable caching
         FsFileInfo_setAllowCaching(fileInfo, fhgfs_false);
      }
...
int FhgfsOps_open(struct inode* inode, struct file* file)
{
   ...
   return FhgfsOps_openReferenceHandle(app, inode, file, openFlags, lookupInfo);
}
...
int FhgfsOps_release(struct inode* inode, struct file* file)
{
   ...
   closeRes = FhgfsInode_releaseHandle(fhgfsInode, handleType);
...
int FhgfsOps_releaseCancelLocks(struct inode* inode, struct file* file)
{
   ...
   { // entry locks were used with this file handle
      ...
      unlockRes = FhgfsOpsHelper_unlockEntryWithAsyncRetry(FhgfsInode_getEntryInfo(fhgfsInode), &ioInfo,
         clientFD);
   ...
   { // current pid used range locking on this inode
      ...
      unlockRes = FhgfsOpsHelper_unlockRangeWithAsyncRetry(FhgfsInode_getEntryInfo(fhgfsInode), &ioInfo,
         ownerPID);
...
int FhgfsOps_flock(struct file* file, int cmd, struct file_lock* fileLock)
{
   ...
   fhgfs_bool useGlobalFileLocks = Config_getTuneUseGlobalFileLocks(cfg);
   ...
   {
      int flushRes = __FhgfsOps_flush(app, file, fhgfs_false, fhgfs_false, fhgfs_true,
         fhgfs_false);
   ...
   if(useGlobalFileLocks)
   {
      ...
      globalLockRes = FhgfsOpsRemoting_flockEntry(FhgfsInode_getEntryInfo(fhgfsInode), &ioInfo,
         (size_t)fileLock->fl_file, fileLock->fl_pid, lockTypeFlags);
...
int FhgfsOps_lock(struct file* file, int cmd, struct file_lock* fileLock)
{
   ...
   if(useGlobalFileLocks &&
      (lockTypeFlags & ENTRYLOCKTYPE_LOCKOPS_REMOVE) )
   {
      int flushRes = __FhgfsOps_flush(app, file, fhgfs_false, fhgfs_false, fhgfs_true, fhgfs_false);
   ...
   if(useGlobalFileLocks)
   {
      ...
      globalLockRes = FhgfsOpsRemoting_flockRange(FhgfsInode_getEntryInfo(fhgfsInode), &ioInfo,
         fileLock->fl_pid, lockTypeFlags, fileLock->fl_start, fileLock->fl_end);
   ...
   loff_t* offsetPointer)
{
   ...
   fhgfs_bool isLocallyLockedAppend =
      FsFileInfo_getAppending(fileInfo) && !Config_getTuneUseGlobalAppendLocks(cfg);
   fhgfs_bool isGloballyLockedAppend =
      FsFileInfo_getAppending(fileInfo) && Config_getTuneUseGlobalAppendLocks(cfg);
   ...
   { // appending without global locks => move file offset to end-of-file before writing
      ...
      flushRes = __FhgfsOps_flush(app, file, fhgfs_false, fhgfs_false, fhgfs_true, fhgfs_false);
      if(unlikely(flushRes < 0) )
      { // flush error
         writeRes = flushRes;
...
int FhgfsOps_fsync(struct file* file, struct dentry* dentry, int datasync)
{
   ...
   // TODO: Implement start and end here as allowed by >linux-3.1

   retVal = __FhgfsOps_flush(app, file, fhgfs_false, Config_getTuneRemoteFSync(cfg), fhgfs_true,
      fhgfs_false);
 ...
 * error (typically you will only want to set fhgfs_true here during file close).
 * @param forceRemoteFlush whether or not remote fsync will be executed depends on this value
 * and the corresponding config value (fsync will use fhgfs_true here and flush won't)
 ...
 */
int __FhgfsOps_flush(App* app, struct file *file, fhgfs_bool discardCacheOnError,
   fhgfs_bool forceRemoteFlush, fhgfs_bool checkSession, fhgfs_bool isClose)
{
   Logger* log = App_getLogger(app);
   Config* cfg = App_getConfig(app);
   const char* logContext = __func__;
