...
extern fhgfs_bool FhgfsOps_initInodeCache(void);
...
static inline fhgfs_bool __FhgfsOps_isPagedMode(struct super_block* sb);
   ...
   if (S_ISREG(inOutInode->i_mode) && FhgfsInode_getHasPageWriteFlag(fhgfsInode) )
   {
      ...
      if (oldSize > kstat->size)
      {
         ...
         if (iSizeHints)
         {
          ...
          #ifdef BEEGFS_DEBUG
          {
             static fhgfs_bool didLog = fhgfs_false;
...
{
      return __FhgfsOps_newInodeWithParentID(sb, kstat, dev, entryInfo, 0, iSizeHints);
}

fhgfs_bool __FhgfsOps_isPagedMode(struct super_block* sb)
{
   App* app = FhgfsOps_getApp(sb);
   Config* cfg = App_getConfig(app);
   ...
   FhgfsIsizeHints* iSizeHints)
{
   ...
   return __FhgfsOps_doRefreshInode(app, inode, fhgfsStat, iSizeHints, fhgfs_false);
}
