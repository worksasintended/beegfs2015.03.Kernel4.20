...
static long FhgfsOpsIoctl_createFile(struct file *file, void __user *argp);
...
long FhgfsOpsIoctl_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
   ...
   switch(cmd)
   {
      ...
      {
         return FhgfsOpsIoctl_createFile(file, (void __user *) arg);
      } break;
...
static long FhgfsOpsIoctl_getRuntimeCfgFile(struct file *file, void __user *argp)
{
   ...
   cfgFileStrLen = os_scnprintf(fileName, BEEGFS_IOCTL_CFG_MAX_PATH, "/proc/fs/beegfs/%s/config",
      localNodeID);
   if (cfgFileStrLen <= 0)
   {
   ...
   cfgFileStrLen++; // os_scnprintf(...) does not count \0
...
long FhgfsOpsIoctl_mkfileWithStripeHints(struct file *file, void __user *argp)
{
   ...
   CreateInfo_init(app, parentInode, filename, mode, fhgfs_true, &createInfo);
...
static long FhgfsOpsIoctl_createFile(struct file *file, void __user *argp)
{
   ...
   struct BeegfsIoctl_MkFile_Arg fileInfo;
   struct CreateInfo createInfo =
   {
      .preferredStorageTargets = NULL,
      .preferredMetaTargets = NULL
   ...
   memset(&fileInfo, 0, sizeof(struct BeegfsIoctl_MkFile_Arg) ); // required for SAFE_KFREEs() below

   retVal = IoctlHelper_ioctlCreateFileCopyFromUser(app, argp, &fileInfo); // user to kernel space
   if(retVal)
   {
   ...
   CreateInfo_init(app, inode, fileInfo.entryName, fileInfo.mode, fhgfs_true,
      &createInfo);
   ...
   EntryInfo_init(&parentInfo, fileInfo.ownerNodeID, fileInfo.parentParentEntryID,
      fileInfo.parentEntryID, fileInfo.parentName, DirEntryType_DIRECTORY, STATFLAG_HINT_INLINE);
