...
void FhgfsInode_allocInit(FhgfsInode* fhgfsInode)
{
   ...
   {
      fhgfsInode->fileHandles[i].needsAppendLockCleanup = fhgfs_false;
      ...
      BitStore_init(&(fhgfsInode->fileHandles[i].firstWriteDone), fhgfs_false);
   }
 ...
 * @param allowRWHandle fhgfs_true if you specified read or write flags and would also accept
 ...
 */
FhgfsOpsErr FhgfsInode_referenceHandle(FhgfsInode* this, int openFlags, fhgfs_bool allowRWHandle,
   LookupIntentInfoOut* lookupInfo, FileHandleType* outHandleType)
{
   ...
   *outHandleType = __FhgfsInode_openFlagsToHandleType(openFlags);
   ...
   { // desired handle exists => return it
      retVal = __FhgfsInode_referenceTrunc(this, app, openFlags);
   ...
   { // no matching handle yet => open file to create a new handle
      ...
      {  // file yet opened by lookup or atomic open
         ...
         retVal = FhgfsOpsRemoting_openfile(entryInfo, &ioInfo);
...
FhgfsOpsErr FhgfsInode_releaseHandle(FhgfsInode* this, FileHandleType handleType)
{
   ...
   if(unlikely(!handle->refCount) )
      BEEGFS_BUG_ON(fhgfs_true, "refCount already 0"); // refcount already zero => bug
   else
   { // positive refCount
      handle->refCount--;
      ...
      { // we dropped the last reference => close remote
         ...
         retVal = FhgfsOpsHelper_closefileWithAsyncRetry(entryInfo, &ioInfo);
         ...
         handle->needsAppendLockCleanup = fhgfs_false;
...
fhgfs_bool FhgfsInode_hasWriteHandle(FhgfsInode* this)
{
 ...
 *
 * @return fhgfs_true if the pid was found (i.e. was previously added via _addRangeLockPID() ).
 */
fhgfs_bool FhgfsInode_removeRangeLockPID(FhgfsInode* this, int pid)
{
   fhgfs_bool pidExisted;
...
FhgfsOpsErr __FhgfsInode_referenceTrunc(FhgfsInode* this, App* app, int openFlags)
{
   ...
   if(openFlags & OPENFILE_ACCESS_TRUNC)
   {
      ...
      retVal = FhgfsOpsRemoting_truncfile(app, entryInfo, 0);
...
fhgfs_bool FhgfsInode_isCacheValid(FhgfsInode* this, umode_t i_mode, Config* cfg)
{
   unsigned cacheValidityMS;
   unsigned cacheElapsedMS;
   fhgfs_bool cacheValid;
