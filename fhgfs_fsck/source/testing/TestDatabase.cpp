...
TestDatabase::TestDatabase()
 : databasePath(Program::getApp()->getConfig()->getTestDatabasePath() )
{
}
...
void TestDatabase::testUpdateFileInodes()
{
   ...
   while (!inodeListIn.empty() )
   {
      ...
      inodeListIn.front().setStripeTargets(empty);
...
void TestDatabase::testFindDuplicateInodeIDs()
{
   ...
   disposal.push_back(
      FsckDirInode("disposal", "", 1, 1, 0, 0, noTargets, FsckStripePatternType_RAID0, 1) );
   disposal.push_back(
      FsckDirInode("disposal", "", 2, 2, 0, 0, noTargets, FsckStripePatternType_RAID0, 2) );
...
void TestDatabase::testCheckForAndInsertOrphanedChunks()
{
   ...
   // create file inodes
   FsckFileInodeList fileInodes;
   DatabaseTk::createDummyFsckFileInodes(NUM_CHUNKS-MISSING_INODES - 1, &fileInodes);

   this->db->getModificationEventsTable()->insert(
      FsckModificationEventList(1,
         FsckModificationEvent(ModificationEvent_DIRMOVED, chunks.rbegin()->getID() ) ),
      this->db->getModificationEventsTable()->newBulkHandle() );
   ...
   chunks.clear();
   drainToList(this->db->findOrphanedChunks(), chunks);
...
void TestDatabase::testCheckForAndInsertFileInodesWithWrongAttribs()
{
   ...
   while ( i < WRONG_DATA_INODES_HARDLINKS )
   {
      ...
      if ( tmpIter == modifiedIDs.end() )
      {
         ...
         inodeIter->setNumHardLinks(i == 0 ? 0 : 100);
         i++;
      }
   ...
   inodeIter->setFileSize(inodeIter->getFileSize() + 1);
   this->db->getModificationEventsTable()->insert(
      FsckModificationEventList(1,
...
void TestDatabase::testCheckForAndInsertFilesWithMissingStripeTargets()
{
   ...
   targetMapper.mapTarget(1, 1);
   targetMapper.mapTarget(2, 1);
   buddyGroupMapper.mapMirrorBuddyGroup(3, 1, 2, 0, NULL);
   ...
   for (FsckFileInodeListIter file = inodes.begin(); file != inodes.end(); file++, i++)
   {
      ...
      file->setStripeTargets(stripeTargets);
      file->setStripePatternType(pattern->front().getTargetIDType() == FsckTargetIDType_BUDDYGROUP
