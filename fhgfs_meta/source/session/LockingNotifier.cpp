...
void LockingNotifier::notifyWaitersEntryLock(LockEntryNotifyType lockType,
   std::string parentEntryID, std::string entryID, LockEntryNotifyList* notifyList)
{
   // just delegate to comm slaves

   MultiWorkQueue* slaveQ = Program::getApp()->getCommSlaveQueue();

   Work* work = new LockEntryNotificationWork(lockType, parentEntryID, entryID, notifyList);
 ...
 * calling this.
 */
void LockingNotifier::notifyWaitersRangeLock(std::string parentEntryID, std::string entryID,
