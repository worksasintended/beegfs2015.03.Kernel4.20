   ...
   unsigned msgUserID)
{
   ...
   FhgfsOpsErr unlinkMetaRes = unlinkMetaFile(parentID, removeName, &unlinkedInode);
   ...
   {
      return unlinkChunkFiles(unlinkedInode, msgUserID);
   }
 ...
 */
FhgfsOpsErr MsgHelperUnlink::unlinkMetaFile(std::string parentID, std::string removeName,
   FileInode** outUnlinkedFile)
{
   ...
   FhgfsOpsErr unlinkMetaRes = metaStore->unlinkFile(parentID, removeName, &entryInfo,
      outUnlinkedFile);
