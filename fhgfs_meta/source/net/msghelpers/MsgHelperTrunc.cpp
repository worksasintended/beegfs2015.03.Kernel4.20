...
FhgfsOpsErr MsgHelperTrunc::truncChunkFile(FileInode* inode, EntryInfo* entryInfo,
   int64_t filesize, bool useQuota, unsigned userIDHint)
{
   ...
   if( (pattern->getStripeTargetIDs()->size() > 1) ||
      ...
      return truncChunkFileParallel(inode, entryInfo, filesize, useQuota, userIDHint);
   else
      return truncChunkFileSequential(inode, entryInfo, filesize, useQuota, userIDHint);
}
   ...
   int64_t filesize, bool useQuota, unsigned msgUserID)
{
   ...
   for(size_t i=0; i < numTargetWorks; i++)
   {
      ...
      TruncChunkFileWork* work = new TruncChunkFileWork(inode->getEntryID(), localTruncPos,
         pattern, (*targetIDs)[i], &pathInfo, &(dynAttribsVec[i]), &(nodeResults[i]), &counter);
