   ...
   char* respBuf, size_t bufLen, HighResolutionStats* stats)
{
   ...
   LOG_DEBUG_CONTEXT(log, Log_DEBUG, std::string("Received a RefreshEntryInfoMsg from: ") + peer);

   LOG_DEBUG("RefreshEntryInfoMsgEx::processIncoming", Log_SPAM, std::string("ParentID: ") +
   ...
   if(this->getEntryInfo()->getParentEntryID().empty() )
   ...
   app->getNodeOpStats()->updateNodeOp(sock->getPeerIP(), MetaOpCounter_REFRESHENTRYINFO,
      getMsgHeaderUserID() );
...
FhgfsOpsErr RefreshEntryInfoMsgEx::refreshInfoRec(EntryInfo* entryInfo)
{
   ...
   return MsgHelperStat::refreshDynAttribs(entryInfo, true, getMsgHeaderUserID() );
}
