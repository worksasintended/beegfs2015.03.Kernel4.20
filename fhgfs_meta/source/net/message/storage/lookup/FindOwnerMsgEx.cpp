   ...
   char* respBuf, size_t bufLen, HighResolutionStats* stats)
{
   ...
   { // looking for the root node
      ...
      if(rootNodeID)
      {
         ownerInfo.set(0, rootNodeID, "", META_ROOTDIR_ID_STR, "/", DirEntryType_DIRECTORY, 0);
   ...
   app->getNodeOpStats()->updateNodeOp(sock->getPeerIP(), MetaOpCounter_FINDOWNER,
      getMsgHeaderUserID() );
...
FhgfsOpsErr FindOwnerMsgEx::findOwner(EntryOwnerInfo* outInfo)
{
   ...
   while(currentDepth < searchDepth)
   {
      ...
      EntryInfo subInfo;
      FhgfsOpsErr getEntryRes = metaStore->getEntryData(currentDir, *pathIter, &subInfo, NULL);
