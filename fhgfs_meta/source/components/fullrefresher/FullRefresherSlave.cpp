...
// TODO also need to refresh inlined inodes
...
void FullRefresherSlave::updateAllMetadata()
{
   ...
   // TODO: Make it work with subDirs again
...
/**
 * Refresh/recalculate metadata of an non-inlined inode.
 *
 * @param entryID ID of the inode
   ...
   uint64_t* inoutNumDirsRefreshed, uint64_t* inoutNumFilesRefreshed)
{
   ...
   bool referenceRes = metaStore->referenceInode(entryID, &fileInode, &dirInode);
   if(!referenceRes)
   ...
   if(fileInode)
   {
      // fake entryInfo (we only need a compatible interface here).
      /* in general, we are accessing non-inlined inodes in the hash directories, so a complete
         entryInfo with a parent and ownerNodeID is not required for access. */

      uint16_t ownerNodeID = app->getLocalNode()->getNumID();
      std::string parentID("REFRESHER_DIRECT_ACCESS");
      std::string fileName("REFRESHER_UKNOWN_FNAME");
      DirEntryType entryType = MetadataTk::posixFileTypeToDirEntryType(fileInode->getMode() );
      int flags = 0;

      EntryInfo entryInfo(ownerNodeID, parentID, entryID, fileName, entryType, flags);

      // release the file, as we do not really need it
      // (parentID can be anything, as the inode is definitely not inlined)
      metaStore->releaseFile(parentID, fileInode);

      MsgHelperStat::refreshDynAttribs(&entryInfo, true, NETMSG_DEFAULT_USERID);
