 ...
 *       a dentry-owner.
 *
 *       @param useDirEntryName defaults to false, only used by fsck and generic-debug.
 */
bool DirEntry::storeUpdatedDirEntry(std::string dirEntryPath, bool useDirEntryName)
{
   char buf[DIRENTRY_SERBUF_SIZE];
   ...
   if (unlikely(useDirEntryName) )
      saveRes = storeUpdatedDirEntryBuf(dirEntryPath + "/" + this->name, buf, bufLen);
   else
   {
      std::string idStorePath = MetaStorageTk::getMetaDirEntryIDPath(dirEntryPath) + getEntryID();

      saveRes = storeUpdatedDirEntryBuf(idStorePath, buf, bufLen);
   }
 ...
 * */
FhgfsOpsErr DirEntry::storeUpdatedInode(std::string dirEntryPath)
{
 ...
 */
FileInode* DirEntry::createInodeByID(std::string dirEntryPath, EntryInfo* entryInfo)
{
