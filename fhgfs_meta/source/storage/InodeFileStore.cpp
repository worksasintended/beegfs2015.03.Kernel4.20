...
FhgfsOpsErr InodeFileStore::unlinkFileInodeUnlocked(EntryInfo* entryInfo, FileInode** outInode)
{
   ...
   bool unlinkRes = FileInode::unlinkStoredInodeUnlocked(entryID);
   if(!unlinkRes)
   {
      if(outInode)
      {
 ...
 * Finish the rename/move operation by deleting the inode object.
 */
void InodeFileStore::deleteUnreferencedInodeUnlocked(std::string entryID)
{
...
bool InodeFileStore::existsUnlocked(std::string fileID)
{
   ...
   std::string metaFilename = MetaStorageTk::getMetaInodePath(
      app->getInodesPath()->getPathAsStrConst(), fileID);
 ...
 * @param parentMirrorNodeID set to mirror ID of parent dir (0 otherwise), used to update mirror
 * for inlined inodes.
 */
FhgfsOpsErr InodeFileStore::setAttr(EntryInfo* entryInfo, int validAttribs,
   SettableFileAttribs* attribs, uint16_t parentMirrorNodeID)
{
   std::string entryID = entryInfo->getEntryID();
   FhgfsOpsErr retVal = FhgfsOpsErr_PATHNOTEXISTS;
   ...
   { // not loaded => load, apply, destroy
      ...
      { // loaded
         ...
         { // attr update succeeded
            ...
            // add entry to mirror queue (via dentry-by-id)
            if(parentMirrorNodeID &&
               inode->getIsInlined() &&
               !entryInfo->getParentEntryID().empty() )
            {
               MetadataMirrorer::addNewDentryStatic(entryInfo->getParentEntryID(),
                  "", entryInfo->getEntryID(), false, parentMirrorNodeID);
            }
   ...
   uint16_t parentMirrorNodeID, int value)
{
   ...
   if (setRes == true && parentMirrorNodeID && inode->getIsInlined() &&
      !entryInfo->getParentEntryID().empty() )
   {
      MetadataMirrorer::addNewDentryStatic(entryInfo->getParentEntryID(), "",
         entryInfo->getEntryID(), false, parentMirrorNodeID);
