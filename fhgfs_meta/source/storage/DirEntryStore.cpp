...
{
   std::string dentryPath = Program::getApp()->getDentriesPath()->getPathAsStrConst();
...
FhgfsOpsErr DirEntryStore::mkDentryStoreDir(std::string dirID)
{
   ...
   std::string contentsDirStr = MetaStorageTk::getMetaDirEntryPath(
      app->getDentriesPath()->getPathAsStrConst(), dirID);
...
bool DirEntryStore::rmDirEntryStoreDir(std::string& id)
{
   ...
   App* app = Program::getApp();
   std::string contentsDirStr = MetaStorageTk::getMetaDirEntryPath(
      app->getDentriesPath()->getPathAsStrConst(), id);
...
{
   std::string dirEntryPath = getDirEntryPathUnlocked();
 ...
 */
FhgfsOpsErr DirEntryStore::unlinkDirEntry(std::string entryName, DirEntry* entry,
 ...
 */
FhgfsOpsErr DirEntryStore::unlinkDirEntryUnlocked(std::string entryName, DirEntry* entry,
   unsigned unlinkTypeFlags)
{
 ...
 */
FhgfsOpsErr DirEntryStore::linkEntryInDir(std::string fromEntryName, std::string toEntryName)
{
   ...
   unsigned maxOutNames, bool filterDots, ListIncExOutArgs& outArgs)
{
   ...
   if(serverOffset)
   {
      //LOG_DEBUG(logContext, Log_SPAM, "seeking to: " + StringTk::uint64ToStr(serverOffset) );
       ...
       numEntries++)
   {
      ...
      //LOG_DEBUG(logContext, Log_SPAM, "filled: " + std::string(dirEntry->d_name) + "; "
      //   "offset: " + StringTk::uint64ToStr(dirEntry->d_off) );
 ...
 *
 */
FhgfsOpsErr DirEntryStore::getEntryData(std::string entryName, EntryInfo* outInfo,
 ...
 */
void DirEntryStore::setParentID(std::string parentID)
{
