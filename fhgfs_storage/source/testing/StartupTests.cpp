...
bool StartupTests::testPath()
{
   ...
   std::string pathStr;
   for(StringListIter iter = origPathElems.begin(); iter != origPathElems.end(); iter++)
   {
      pathStr += slash + *iter;
   }
   ...
   if(pathElems->size() != origPathElems.size() )
   {
      log.logErr(std::string("Element count is wrong: ") +
         StringTk::intToStr(pathElems->size() ) );
      return false;
   }
   
   StringListConstIter iterOrig = origPathElems.begin();
   StringListConstIter iter = pathElems->begin();
   for( ; iter != pathElems->end(); iter++, iterOrig++)
   {
      if(iter->compare(*iterOrig) )
      {
         log.logErr("Strings do not match: " +
            std::string("'") + *iter + std::string("'") +
            std::string(" / ") +
            std::string("'") + *iterOrig + std::string("'") );
            
         return false;
      }
      
      log.log(3, std::string("pathElem: '") + *iter + std::string("'") ); 
   }
