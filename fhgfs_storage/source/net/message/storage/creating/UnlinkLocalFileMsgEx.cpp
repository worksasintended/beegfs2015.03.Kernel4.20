...
bool UnlinkLocalFileMsgEx::processIncoming(struct sockaddr_in* fromAddr, Socket* sock,
   char* respBuf, size_t bufLen, HighResolutionStats* stats)
{
   const char* logContext = "UnlinkChunkFileMsg incoming";
      ...
      LOG_DEBUG(logContext, Log_DEBUG, std::string("Received a UnlinkLocalFileMsg from: ") + peer);
   #endif // BEEGFS_DEBUG
...
send_response:
   ...
   { // send response...
      UnlinkLocalFileRespMsg respMsg(clientErrRes);
...
skip_response:
   ...
   app->getNodeOpStats()->updateNodeOp(
      sock->getPeerIP(), StorageOpCounter_UNLINK, getMsgHeaderUserID() );
 ...
 * dir).
 */
int UnlinkLocalFileMsgEx::getTargetFD(struct sockaddr_in* fromAddr, Socket* sock,
 ...
 */
FhgfsOpsErr UnlinkLocalFileMsgEx::forwardToSecondary(struct sockaddr_in* fromAddr, Socket* sock,
