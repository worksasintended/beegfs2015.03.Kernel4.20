...
#include <net/msghelpers/MsgHelperHsm.h>
...
bool TruncLocalFileMsgEx::processIncoming(struct sockaddr_in* fromAddr, Socket* sock,
   char* respBuf, size_t bufLen, HighResolutionStats* stats)
{
   const char* logContext = "TruncChunkFileMsg incoming";
      ...
      LOG_DEBUG(logContext, Log_DEBUG, std::string("Received a TruncLocalFileMsg from: ") + peer);
   #endif // BEEGFS_DEBUG
   ...
   { // valid targetID
      ...
      clientErrRes = truncFile(targetFD, &chunkDirPath, chunkFilePathStr, entryID, hasOrigFeature);
      ...
      { // truncation successful
         ...
         // notify the HSM server

         // TODO: Do we really need to send if FhgfsOpsErr_PATHNOTEXISTS? Probably not.

         bool sendRes = MsgHelperHsm::sendArchiveFilesMsg(targetID, entryID,
            HSM_COLLOCATIONID_INVALID);

         if (!sendRes)
            log.logErr("Could not send archive files msg to HSM node.");
#endif
      }
   }
...
send_response:
   ...
   { // send response...
      TruncLocalFileRespMsg respMsg(clientErrRes, dynAttribs.filesize, dynAttribs.allocedBlocks,
         dynAttribs.modificationTimeSecs, dynAttribs.lastAccessTimeSecs, dynAttribs.storageVersion);
...
skip_response:
   ...
   app->getNodeOpStats()->updateNodeOp(
      sock->getPeerIP(), StorageOpCounter_TRUNCLOCALFILE, getMsgHeaderUserID() );
 ...
 * dir).
 */
int TruncLocalFileMsgEx::getTargetFD(struct sockaddr_in* fromAddr, Socket* sock,
...
FhgfsOpsErr TruncLocalFileMsgEx::truncFile(int targetFD, PathVec* chunkDirPath,
   std::string& chunkFilePathStr, std::string entryID, bool hasOrigFeature)
{
   ...
   { // error
      clientErrRes = fhgfsErrFromSysErr(truncErrCode);
   ...
   FhgfsOpsErr mkChunkRes = chunkDirStore->openChunkFile(
   ...
   { // error
      clientErrRes = fhgfsErrFromSysErr(errno);
      if (clientErrRes == FhgfsOpsErr_INTERNAL) // only log unhandled errors
 ...
 */
FhgfsOpsErr TruncLocalFileMsgEx::forwardToSecondary(struct sockaddr_in* fromAddr, Socket* sock,
 ...
 */
FhgfsOpsErr TruncLocalFileMsgEx::fhgfsErrFromSysErr(int errCode)
{
   switch(errCode)
   {
      case 0:
