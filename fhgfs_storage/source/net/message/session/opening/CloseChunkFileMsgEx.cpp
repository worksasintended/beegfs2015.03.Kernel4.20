...
bool CloseChunkFileMsgEx::processIncoming(struct sockaddr_in* fromAddr, Socket* sock,
   char* respBuf, size_t bufLen, HighResolutionStats* stats)
{
   const char* logContext = "CloseChunkFileMsg incoming";
      ...
      LOG_DEBUG(logContext, Log_DEBUG, std::string("Received a CloseChunkFileMsg from: ") + peer);
   #endif // BEEGFS_DEBUG
   ...
   closeMsgRes = forwardToSecondary(fromAddr, sock, respBuf, bufLen);
   ...
   if(sessionLocalFile && sessionLocalFile->getMirrorNode() )
   {
   ...
   // note: "file not exists" is not an error. we just have nothing to do in that case.
    ...
    // notify the HSM server, but only if data was actually written to the file
   if (sessionLocalFile && sessionLocalFile->getDataHasChanged())
   {
      std::string entryID = SessionTk::fileIDFromHandleID(fileHandleID);
      bool sendRes = MsgHelperHsm::sendArchiveFilesMsg(targetID, entryID, getHsmCollocationID());
      if (!sendRes)
         LogContext(logContext).logErr("Could not send archive files msg to HSM node.");
   }
#endif
...
send_response:
   ...
   {
      CloseChunkFileRespMsg respMsg(closeMsgRes, dynAttribs.filesize, dynAttribs.allocedBlocks,
...
skip_response:
   ...
   app->getNodeOpStats()->updateNodeOp(
      sock->getPeerIP(), StorageOpCounter_CLOSELOCAL, getMsgHeaderUserID() );
 ...
 */
FhgfsOpsErr CloseChunkFileMsgEx::forwardToSecondary(struct sockaddr_in* fromAddr, Socket* sock,
