...
bool WriteLocalFileMsgEx::processIncoming(struct sockaddr_in* fromAddr, Socket* sock,
   char* respBuf, size_t bufLen, HighResolutionStats* stats)
{
   const char* logContext = "WriteChunkFileMsg incoming";
      ...
      LOG_DEBUG(logContext, Log_DEBUG,
         std::string("Received a WriteLocalFileMsg from: ") + peer);

//      LOG_DEBUG("WriteChunkFileMsg incoming", Log_SPAM,
//         std::string("Values for sessionID/FD/offset/count: ") + std::string(getSessionID() ) +
//         "/" + StringTk::intToStr(getFD() ) + "/" + StringTk::intToStr(getOffset() ) + "/" +
//         StringTk::intToStr(getCount() ) );
   #endif // BEEGFS_DEBUG
      ...
      app->getConfig()->getQuotaEnableEnforcement() )
   {
      ...
      if(quotaExceeded != QuotaExceededErrorType_NOT_EXCEEDED)
      {
         ...
         incrementalRecvPadding(sock, respBuf, bufLen, getCount(), sessionLocalFile);
   ...
   try
   {
      ...
      FhgfsOpsErr prepMirrorRes = prepareMirroring(respBuf, bufLen, sessionLocalFile);
      ...
      int64_t writeLocalRes = incrementalRecvAndWriteStateful(
         sock, respBuf, bufLen, sessionLocalFile);
      ...
      { // writing succeeded
         ...
         stats->incVals.diskWriteBytes += writeLocalRes; // update stats

#ifdef BEEGFS_HSM_DEPRECATED
         // set info, that file has changed in session
         sessionLocalFile->setDataHasChanged(true);
#endif
      }
   ...
   catch(SocketException& e)
   {
      ...
      finishMirroring(respBuf, bufLen, sessionLocalFile);
...
cleanup:
   ...
   finishMirroringRes = finishMirroring(respBuf, bufLen, sessionLocalFile);
   ...
   WriteLocalFileRespMsg respMsg(writeClientRes);
   ...
   if(likely(writeClientRes > 0) )
      app->getNodeOpStats()->updateNodeOp(sock->getPeerIP(), StorageOpCounter_WRITEOPS,
 ...
 */
int64_t WriteLocalFileMsgEx::incrementalRecvAndWriteStateful(Socket* sock,
   char* buf, ssize_t bufLen, SessionLocalFile* sessionLocalFile)
{
   ...
   const ssize_t exactStaticRecvSize = sessionLocalFile->getIsDirectIO() ?
      bufLen : BEEGFS_MIN(bufLen, cfg->getTuneFileWriteSize() );
   ...
   do
   {
      ...
      ssize_t recvRes = sock->recvExactT(buf, recvLength, 0, timeoutMS);

      // forward to mirror...

      FhgfsOpsErr mirrorRes = sendToMirror(buf, recvRes, writeOffset, toBeReceived,
         sessionLocalFile);
      if(unlikely(mirrorRes != FhgfsOpsErr_SUCCESS) )
      { // mirroring failed
         incrementalRecvPadding(sock, buf, bufLen, toBeReceived, sessionLocalFile);
      ...
      ssize_t writeRes = unlikely(isMsgHeaderFeatureFlagSet(WRITELOCALFILEMSG_FLAG_DISABLE_IO) ) ?
         recvRes : doWrite(fd, buf, recvRes, writeOffset, errCode);
      ...
      { // didn't write all of the received data
         ...
         { // write error occurred
            ...
            return -fhgfsErrFromSysErr(errCode);
         }
         else
 ...
 */
void WriteLocalFileMsgEx::incrementalRecvPadding(Socket* sock, char* buf, size_t bufLen,
   int64_t padLen, SessionLocalFile* sessionLocalFile)
{
   ...
   while(toBeReceived)
   {
      ssize_t recvRes = sock->recvT(buf, BEEGFS_MIN(toBeReceived, bufLen), 0, timeoutMS);

      // forward to mirror...

      FhgfsOpsErr mirrorRes = sendToMirror(buf, recvRes, getOffset() + padLen - toBeReceived,
         toBeReceived, sessionLocalFile);
      if(unlikely(mirrorRes != FhgfsOpsErr_SUCCESS) )
      { // mirroring failed
   ...
   SessionLocalFile* sessionLocalFile)
{
   ...
   Node* mirrorToNode = sessionLocalFile->getMirrorNode();
   ...
   for( ; ; )
   {
      try
      {
         ...
         WriteLocalFileMsg mirrorWriteMsg(getSessionID(), getFileHandleID(), getTargetID(),
         ...
         mirrorWriteMsg.serialize(buf, bufLen);
         mirrorToSock->send(buf, mirrorWriteMsg.getMsgLength(), 0);
   ...
   for( ; ; )
   {
      try
      {
         ...
         { // retry requires reconnect and resend of write msg with current offset
            ...
            WriteLocalFileMsg mirrorWriteMsg(getSessionID(), getFileHandleID(),
            ...
            unsigned mirrorMsgLen = mirrorWriteMsg.getMsgLength();
            mirrorMsgBuf = (char*) malloc(mirrorMsgLen);

            mirrorWriteMsg.serialize(mirrorMsgBuf, mirrorMsgLen);
            mirrorToSock->send(mirrorMsgBuf, mirrorMsgLen, 0);
 ...
 */
FhgfsOpsErr WriteLocalFileMsgEx::fhgfsErrFromSysErr(int64_t errCode)
{
   switch(errCode)
   {
      case 0:
