 ...
 */
FhgfsOpsErr ChunkStore::fhgfsErrFromSysErr(int errCode)
{
   switch(errCode)
   {
      case 0:
 ...
 */
bool ChunkStore::rmdirChunkDirPath(int targetFD, PathVec* chunkDirPath)
{
   ...
   int chunkDirPos = chunkDirPath->getNumPathElems() - 1;

   // Iterate over all path elements in reverse order and try to rmdir up to uidXYZ
   while (chunkDirPos >= uidPos)
   {
      std::string chunkDirID = getUniqueDirID(chunkDirPath->getLastElem(), chunkDirPos);
      ...
      std::string rmDirPath = chunkDirPath->getPathAsStr();
      ...
      chunkDirPath->popLastElem(); // next upper path level
      chunkDirPos = chunkDirPath->getNumPathElems() - 1;
   }
 ...
 */
bool ChunkStore::mkdirV2ChunkDirPath(int targetFD, PathVec* chunkDirPath)
{
   const char* logContext = "ChunkDirStore mkdir V2 chunkdir path";
   bool retVal = true;
   const StringVector* pathElems = chunkDirPath->getPathElems();
   StringVectorConstIter pathElemsIter = pathElems->begin();

   std::string mkdirPath;

   // Iterate and create basic chunks or mirror paths, we don't need any locking here
   while (pathElemsIter != pathElems->end() )
   {
      mkdirPath += *pathElemsIter;
 ...
 */
bool ChunkStore::mkdirChunkDirPath(int targetFD, PathVec* chunkDirPath, bool hasOrigFeature,
   ChunkDir** outChunkDir)
{
   ...
   *outChunkDir = NULL;
   ...
   while (depth < uidPos && pathElemsIter != pathElems->end() )
   {
      mkdirPath += *pathElemsIter;
...
int ChunkStore::openAndChown(const int targetFD, const std::string& path,
   const int openFlags, const SessionQuotaInfo& quota)
{
   ...
   {
      LogContext(__func__).logErr("Failed to chown(). path: " + path);
      close(fd);
      unlink(path.c_str());
      return -1;
   }
 ...
 */
FhgfsOpsErr ChunkStore::openChunkFile(int targetFD, PathVec* chunkDirPath,
   ...
   SessionQuotaInfo* quotaInfo)
{
   ...
   fd = openAndChown(targetFD, chunkFilePathStr, openFlags, *quotaInfo);
   if (fd == -1)
   { // hash dir didn't exist yet or real error?
      if(errno == ENOENT)
      {  // hash dir just didn't exist yet => create it and open again
         PathVec chunkDirPathTmp;
         if (!chunkDirPath)
         {
            chunkDirPathTmp.parseStr(chunkFilePathStr);
            chunkDirPathTmp.popLastElem();
      ...
      {  // error
         ...
         retVal = fhgfsErrFromSysErr(errno);
         goto out;
      }
   }
 ...
 */
bool ChunkStore::chmodV2ChunkDirPath(int targetFD, PathVec* chunkDirPath, std::string& entryID)
{
   ...
   // Iterate and create basic chunks or mirror paths, we don't need any locking here
   while ((pathElemsIter != pathElems->end() ) || (didEntryID == false) )
   {

      if (pathElemsIter != pathElems->end() )
         chmodPath += *pathElemsIter;
      else
      {
         chmodPath += entryID;
         didEntryID = true;
