...
void StorageTargets::initTargetPathsList(Config* cfg) throw(InvalidConfigException)
{
       ...
       /* conditional iter inc inside loop */)
   {
      ...
      if(currentPath.getIsEmpty() )
      { // empty path => remove
         iter = targetPathsList.erase(iter);
         continue;
      }

      *iter = currentPath.getPathAsStr(); // normalize path

      if(!currentPath.isAbsolute() ) /* (check to avoid problems after chdir) */
...
void StorageTargets::checkBuddyNeedsResync()
{
        ...
        buddyGroupIter != buddyGroupMap.end(); ++buddyGroupIter)
   {
          ...
          CombinedTargetState(TargetReachabilityState_ONLINE, TargetConsistencyState_NEEDS_RESYNC) )
      {
         ...
         FhgfsOpsErr resyncRes = buddyResyncer->startResync(targetID, isNewJob, jobStats);
   ...
   bool targetIDIsBuddy)
{
   ...
   SetTargetConsistencyStatesMsg msg(&targetIDList, &stateList, false);
...
bool StorageTargets::addTargetDir(uint16_t targetNumID, std::string storagePathStr)
{
   ...
   { // successfully opened
      ...
      storageTargetData.targetPath = storagePath.getPathAsStr();

      // Check if target was cleanly shut down.
      sessionFileExists = StorageTk::checkStorageSessionsFileExists(storagePath.getPathAsStr() );
...
bool StorageTargets::removeTargetDir(std::string storagePathStr, bool removeAll)
{
   ...
   Path storagePath(storagePathStr); // to get a normalized path string
   std::string normalizedStoragePath(storagePath.getPathAsStr() );
...
{
   UInt16ListConstIter groupIDIter = buddyGroupIDs.begin();
   UInt16ListConstIter primaryIDIter = primaryTargetIDs.begin();
   UInt16ListConstIter secondaryIDIter = secondaryTargetIDs.begin();

   TargetBuddyGroupMap newTargetBuddyGroupMap;

   for ( ; groupIDIter != buddyGroupIDs.end(); ++groupIDIter, ++primaryIDIter, ++secondaryIDIter)
   {
      uint16_t groupID = *groupIDIter;
      newTargetBuddyGroupMap.insert(std::pair<uint16_t, uint16_t>(*primaryIDIter, groupID) );
      newTargetBuddyGroupMap.insert(std::pair<uint16_t, uint16_t>(*secondaryIDIter, groupID) );
   }
   ...
   TargetStateMap& outStateMap)
{
   ...
   UInt8ListConstIter reachabilityStateIter = reachabilityStates.begin();
   UInt8ListConstIter consistencyStateIter = consistencyStates.begin();

   for ( ; targetIDIter != targetIDs.end();
      ++targetIDIter, ++reachabilityStateIter, ++consistencyStateIter)
   {
      outStateMap.insert(TargetStateMapVal(*targetIDIter, CombinedTargetState(
            static_cast<TargetReachabilityState>(*reachabilityStateIter),
            static_cast<TargetConsistencyState>(*consistencyStateIter)
         ) ) );
   }
}
