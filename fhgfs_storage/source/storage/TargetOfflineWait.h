...
class TargetOfflineWait
{
   public:
      TargetOfflineWait(Config* cfg)
         : waitTimeoutMS(calculateOfflineWaitTimeoutMS(cfg) )
         {
         }
   ...
   private:
       ...
       *  2 * update interval:
       *      until target gets pofflined
       *      end of offline timeout until next target state update (worst case)
       *  plus the actual target offline timeout.
       */
      static unsigned calculateOfflineWaitTimeoutMS(Config* cfg)
      {
         return (
            ( 5
            + 4 * cfg->getSysUpdateTargetStatesSecs()
