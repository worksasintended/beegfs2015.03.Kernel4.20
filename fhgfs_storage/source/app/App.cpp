...
void App::runNormal()
{
   ...
   { // no local num ID yet => try to retrieve one from mgmt
      bool preregisterNodeRes = preregisterNode();
   ...
   bool preregisterTargetsRes = preregisterTargets();
   ...
   InternodeSyncer::syncClientSessions(&emptyClientsList);
...
void App::initDataObjects() throw(InvalidConfigException)
{
   ...
   this->hsmNodes = new NodeStoreEx(NODETYPE_Hsm);
...
void App::initLocalNodeIDs() throw(InvalidConfigException)
{
   ...
   for(StringListIter iter = targetPaths.begin(); iter != targetPaths.end(); iter++)
      StorageTk::checkOrCreateOrigNodeIDFile(*iter, localNodeID);
...
void App::initLocalNodeNumIDFile() throw(InvalidConfigException)
{
   ...
   for(StringListIter iter = targetPaths.begin(); iter != targetPaths.end(); iter++)
   {
      StorageTk::createNumIDFile(*iter, STORAGETK_NODENUMID_FILENAME, localNodeNumID);
   }
...
void App::initComponents() throw(ComponentInitException)
{
   ...
   this->heartbeatMgr = new HeartbeatManager();
...
void App::stopComponents()
{
   ...
   if (buddyResyncer)
      buddyResyncer->stopAllJobs(); // ignored if not running
...
void App::joinComponents()
{
   ...
   // the buddyResyncer is not a normal component, so it gets special treatment here
   if(buddyResyncer)
      buddyResyncer->waitForStopAllJobs();
...
bool App::preregisterNode()
{
   ...
   RegisterNodeMsg msg(localNodeID.c_str(), localNodeNumID, NODETYPE_Storage, &localNicList,
...
bool App::registerAndDownloadMgmtInfo()
{
   ...
   do
   {
      ...
      if(!HeartbeatManager::registerNode(&regDGramLis) ||
         !HeartbeatManager::registerTargetMappings() )
         continue;
