   ...
   EntryInfo* outEntryInfo)
{
   ...
   char* buf = (char*)malloc(MAX_SERIAL_ENTRYINFO_SIZE);
   ssize_t getRes = MsgHelperIO::getXattrAt(targetFD, filepath.c_str(), CHUNK_XATTR_ENTRYINFO_NAME,
      buf, MAX_SERIAL_ENTRYINFO_SIZE);

   if (getRes > 0)
   {
      unsigned deserializeOutLen;
      bool serialRes = outEntryInfo->deserialize(buf, MAX_SERIAL_ENTRYINFO_SIZE,
         &deserializeOutLen);
      free(buf);
      return serialRes;
   }
