...
#define IBVSOCKET_MAX_BUF_SIZE_NUM                    131072 // num * size <= 128MiB
...
void IBVSocket_init(IBVSocket* _this)
{
   ...
   if(!_this->cm_channel)
   {
      SyslogLogger::log(LOG_WARNING,
         "%d:%s: rdma_create_event_channel failed\n", __LINE__, __func__);
      return;
   }
   ...
   IBVCommContext* commContext)
{
   ...
   if(!_this->cm_channel)
   {
      SyslogLogger::log(LOG_WARNING, "%s:%d: rdma_create_event_channel failed\n", __func__, __LINE__);
      return;
   }
   ...
   IBVCommConfig* commCfg)
{
   ...
   if(getInfoRes < 0)
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: %s for %s:%d\n",
         __LINE__, __func__, gai_strerror(getInfoRes), hostname, (int)port);
...
bool IBVSocket_connectByIP(IBVSocket* _this, struct in_addr* ipaddress, unsigned short port,
   IBVCommConfig* commCfg)
{
   const char* logContext = "IBVSocket (connect)";
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: rdma_resolve_addr failed\n",
         __LINE__, __func__);
   ...
   {
      syslog_fhgfs_connerr(stderr, "%d:%s: unexpected CM event %d: %s\n",
         __LINE__, __func__, event->event, rdma_event_str(event->event) );
   ...
   if (_this->typeOfService)
   {
      ...
      {
         SyslogLogger::log(LOG_WARNING, "%d:%s: Failed to set Type Of Service: %hu\n",
            __LINE__, __func__, _this->typeOfService );
         goto err_invalidateSock;
      }
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%s: rdma_resolve_route failed\n", logContext);
      goto err_invalidateSock;
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: unexpected CM event (%d: %s)\n",
         __LINE__, __func__, event->event, rdma_event_str(event->event) );
      goto err_ack_and_invalidateSock;
   }
   ...
   if(!createContextRes)
   {
      SyslogLogger::log(LOG_WARNING, "%s:%d: creation of CommContext failed\n",
         __func__, __LINE__);
      goto err_invalidateSock;
   }
   ...
   {
      syslog_fhgfs_connerr(stderr, "%s: rdma_connect failed\n", logContext);
      goto err_invalidateSock;
   }
   ...
   if(event->event != RDMA_CM_EVENT_ESTABLISHED)
   {
      if(event->event == RDMA_CM_EVENT_REJECTED)
         syslog_fhgfs_connerr(stderr, "%s: connection rejected\n", logContext);
      else
         SyslogLogger::log(LOG_WARNING, "%s: unexpected conn manager event (%d: %s)\n",
            logContext, event->event, rdma_event_str(event->event) );
      goto err_ack_and_invalidateSock;
   }
   ...
   if(!parseCommDestRes)
   {
      SyslogLogger::log(LOG_WARNING, "%s: bad private data received. len: %d\n",
         logContext, event->param.conn.private_data_len);
      goto err_ack_and_invalidateSock;
   }
...
bool IBVSocket_listen(IBVSocket* _this)
{
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: rdma_listen failed\n", __LINE__, __func__);
      goto err_invalidateSock;
   }
   ...
   switch(event->event)
   {
      case RDMA_CM_EVENT_CONNECT_REQUEST:
      {
         ...
         { // bad private data => reject connection
            ...
            SyslogLogger::log(LOG_WARNING, "%d:%s: bad private data received. len: %d\n",
               __LINE__, __func__, event->param.conn.private_data_len);

            if(rdma_reject(child_cm_id, NULL, 0) )
               SyslogLogger::log(LOG_WARNING, "%d:%s: rdma_reject failed\n", __LINE__, __func__);
         ...
         if(!createContextRes)
         {
            SyslogLogger::log(LOG_WARNING, "%d:%s: creation of CommContext failed\n",
               __LINE__, __func__);
         ...
         {
            SyslogLogger::log(LOG_WARNING, "%s:%d: rdma_accept failed\n", __func__, __LINE__);
      ...
      case RDMA_CM_EVENT_ESTABLISHED:
      {
         ...
         *outAcceptedSock = (IBVSocket*)event->id->context;
         ...
         { // events waiting => don't migrate
            SyslogLogger::log(LOG_WARNING,
               "%s:%d: skipping rdma_migrate_id due to waiting events "
               "(but we can live without it).", __func__, __LINE__);
         }
         else
         { // migrate cm_id from general accept-channel to its own channel
            ...
            {
               SyslogLogger::log(LOG_WARNING,
                  "%s:%d: rdma_migrate_id failed (but we can live without it). "
                  "Returned: %d (SysErr: %s)\n", __func__, __LINE__, migrateRes, strerror(errno) );
            }
         }
      ...
      case RDMA_CM_EVENT_DISCONNECTED:
      {
         ...
         syslog_fhgfs_connerr(stdout, "%s:%d: Disconnect event (%d: %s)\n",
            __func__, __LINE__, event->event, rdma_event_str(event->event) );
      ...
      {
         SyslogLogger::log(LOG_WARNING,
            "%s:%d: connect error event while waiting for 'established': %d (%s)\n",
            __func__, __LINE__, event->event, rdma_event_str(event->event) );
      ...
      { // ignore other events
         ...
         { // log only with enabled debug code
            syslog_fhgfs_connerr(stderr, "%s:%d: Ignoring conn manager event (%d: %s)\n",
               __func__, __LINE__, event->event, rdma_event_str(event->event) );
         }
         else
      #endif // SYSTEM_HAS_RDMA_CM_EVENT_TIMEWAIT_EXIT
         { // always log
            SyslogLogger::log(LOG_WARNING, "%s:%d: Ignoring conn manager event (%d: %s)\n",
               __func__, __LINE__, event->event, rdma_event_str(event->event) );
         }
...
bool IBVSocket_shutdown(IBVSocket* _this)
{
   ...
   { // wait for all incomplete sends
      ...
      if(waitRes < 0)
      {
         SyslogLogger::log(LOG_WARNING, "%s: Waiting for incomplete send requests failed\n",
            __func__);
         return false;
      }
   }
   ...
   struct ibv_mr** outMR)
{
   ...
   *outMR = ibv_reg_mr(commContext->pd, buf, bufLen, accessFlags);
   if(!*outMR)
   {
      SyslogLogger::log(LOG_WARNING, "%s:%d: Couldn't allocate MR\n", __func__, __LINE__);
      return -1;
   }
   ...
   struct ibv_mr** outMR)
{
   ...
   if(allocRes)
   {
      SyslogLogger::log(LOG_WARNING, "%s:%d: Couldn't allocate work buf.\n", __func__, __LINE__);
      return NULL;
   }
   ...
   IBVCommConfig* commCfg, IBVCommContext** outCommContext)
{
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%s: bufNum too small (%d vs %d) !\n",
         __func__, commCfg->bufNum, IBVSOCKET_MIN_BUF_NUM);
      goto err_cleanup;
   }

   if (unlikely(commCfg->bufSize < IBVSOCKET_MIN_BUF_SIZE) ) // sanity check
   {
      SyslogLogger::log(LOG_WARNING, "%s: bufSize too small (%d vs %d) !\n",
         __func__, commCfg->bufNum, IBVSOCKET_MIN_BUF_SIZE);
      goto err_cleanup;
   }

   if (unlikely(commCfg->bufSize * commCfg->bufNum) > IBVSOCKET_MAX_BUF_SIZE_NUM)
   {
      SyslogLogger::log(LOG_WARNING, "%s: bufSize*bufNum too large (%d vs %d) !\n",
         __func__, commCfg->bufSize * commCfg->bufNum, IBVSOCKET_MAX_BUF_SIZE_NUM);
      goto err_cleanup;
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: Unbound cm_id!!\n", __LINE__, __func__);
      goto err_cleanup;
   }
   ...
   if(!commContext->pd)
   {
      SyslogLogger::log(LOG_WARNING, "%s:%d: Couldn't allocate PD\n", __func__, __LINE__);
      goto err_cleanup;
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%s:%d: Couldn't prepare recvBuf\n", __func__, __LINE__);
      goto err_cleanup;
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%s:%d: Couldn't prepare sendBuf\n", __func__, __LINE__);
      goto err_cleanup;
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: Couldn't register control memory region\n",
         __LINE__, __func__);
      goto err_cleanup;
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: Couldn't register control memory reset region\n",
         __LINE__, __func__);
      goto err_cleanup;
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: Couldn't create comp channel\n", __LINE__, __func__);
      goto err_cleanup;
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: Couldn't create recv CQ\n", __LINE__, __func__);
      goto err_cleanup;
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: Couldn't create send CQ\n", __LINE__, __func__);
      goto err_cleanup;
   }
   ...
   for(i=0; i < commCfg->bufNum; i++)
   {
      ...
      {
         SyslogLogger::log(LOG_WARNING, "%s: Couldn't post recv buffer with index %d\n",
            __func__, i);
         goto err_cleanup;
      }
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%s: Couldn't request CQ notification\n", __func__);
      goto err_cleanup;
   }
...
void __IBVSocket_cleanupCommContext(struct rdma_cm_id* cm_id, IBVCommContext* commContext)
{
   ...
   if(commContext->sendCQ)
   {
      if(ibv_destroy_cq(commContext->sendCQ) )
         SyslogLogger::log(LOG_WARNING, "%s: Failed to destroy sendCQ\n", __func__);
   }

   if(commContext->recvCQ)
   {
      if(ibv_destroy_cq(commContext->recvCQ) )
         SyslogLogger::log(LOG_WARNING, "%s: Failed to destroy recvCQ\n", __func__);
   }

   if(commContext->recvCompChannel)
   {
      if(ibv_destroy_comp_channel(commContext->recvCompChannel) )
         SyslogLogger::log(LOG_WARNING, "%s: Failed to destroy recvCompChannel\n", __func__);
   }

   if(commContext->controlMR)
   {
      if(ibv_dereg_mr(commContext->controlMR) )
         SyslogLogger::log(LOG_WARNING, "%s: Failed to deregister controlMR\n", __func__);
   }

   if(commContext->controlResetMR)
   {
      if(ibv_dereg_mr(commContext->controlResetMR) )
         SyslogLogger::log(LOG_WARNING, "%s: Failed to deregister controlResetMR\n", __func__);
   }

   if(commContext->recvMR)
   {
      if(ibv_dereg_mr(commContext->recvMR) )
         SyslogLogger::log(LOG_WARNING, "%s: Failed to deregister recvMR\n", __func__);
   }

   if(commContext->sendMR)
   {
      if(ibv_dereg_mr(commContext->sendMR) )
         SyslogLogger::log(LOG_WARNING, "%s: Failed to deregister sendMR\n", __func__);
   }
   ...
   if(commContext->pd)
   {
      if(ibv_dealloc_pd(commContext->pd) )
         SyslogLogger::log(LOG_WARNING, "%s: Failed to dealloc pd\n", __func__);
   }
...
void __IBVSocket_initCommDest(IBVCommContext* commContext, IBVCommDest* outDest)
{
   ...
   Serialization::serializeUInt64( (char*)&outDest->protocolVersion,
      IBVSOCKET_PRIVATEDATA_PROTOCOL_VER);

   //outDest->rkey = commContext->controlMR->rkey;
   Serialization::serializeUInt( (char*)&outDest->rkey, commContext->controlMR->rkey);

   //outDest->vaddr = (uintptr_t)&commContext->numUsedSendBufs;
   Serialization::serializeUInt64( (char*)&outDest->vaddr,
      (uintptr_t)&commContext->numUsedSendBufs);

   //outDest->recvBufNum = commContext->commCfg.bufNum;
   Serialization::serializeUInt( (char*)&outDest->recvBufNum, commContext->commCfg.bufNum);

   //outDest->recvBufSize = commContext->commCfg.bufSize;
   Serialization::serializeUInt( (char*)&outDest->recvBufSize, commContext->commCfg.bufSize);
}
...
bool __IBVSocket_parseCommDest(const void* buf, size_t bufLen, IBVCommDest** outDest)
{
   ...
   *outDest = NULL;
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: Bad private data size. length: %d.\n",
   ...
   Serialization::deserializeUInt64(
      (const char*)&dest->protocolVersion, 8L, &destProtoVer, &destProtoLen);
...
int __IBVSocket_postRecv(IBVSocket* _this, IBVCommContext* commContext, size_t bufIndex)
{
   ...
   if(unlikely(postRes) )
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: ibv_post_recv failed. ErrCode: %d (SysErr: %s)\n",
         __LINE__, __func__, postRes, strerror(errno) );
   ...
   struct ibv_mr* localMR, char* localBuf, int bufLen)
{
   ...
   if(unlikely(postRes) )
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: ibv_post_send() failed: %s\n",
         __LINE__, __func__, strerror(-postRes) );
...
int __IBVSocket_postSend(IBVSocket* _this, size_t bufIndex, int bufLen)
{
   ...
   if(unlikely(postRes) )
   {
      SyslogLogger::log(LOG_WARNING, "%d:%s: ibv_post_send() failed. ErrCode %d (SysErr: %s)\n",
         __LINE__, __func__, postRes, strerror(errno) );
...
int __IBVSocket_recvWC(IBVSocket* _this, int timeoutMS, struct ibv_wc* outWC)
{
   ...
   { // (note: waitRes==0 can often happen, because we call this with timeoutMS==0)
      ...
      if(unlikely(waitRes < 0) )
         syslog_fhgfs_connerr(stderr,
            "%s:%d: retrieval of completion event failed. result: %d\n",
            __func__, __LINE__, waitRes);
      else
      if(unlikely(timeoutMS) )
         syslog_fhgfs_connerr(stderr, "%s:%d: waiting for recv completion timed out.\n",
            __func__, __LINE__);
   ...
   {
      syslog_fhgfs_connerr(stderr, "%s: Connection error (wc_status: %d; msg: %s)\n",
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%s: Completion for unknown/invalid wr_id %d\n",
...
int __IBVSocket_flowControlOnRecv(IBVSocket* _this, int timeoutMS)
{
   ...
   // send control packet if recv counter expires...
      ...
      if(!commContext->numReceivedBufsLeft)
         SyslogLogger::log(LOG_WARNING, "%s: BUG: numReceivedBufsLeft underflow!\n", __func__);
   #endif // BEEGFS_DEBUG
...
void __IBVSocket_flowControlOnSendUpdateCounters(IBVSocket* _this)
{
   ...
   if(!commContext->numSendBufsLeft)
      SyslogLogger::log(LOG_WARNING, "%s: BUG: numSendBufsLeft underflow!\n", __func__);
...
int __IBVSocket_flowControlOnSendWait(IBVSocket* _this, int timeoutMS)
{
   ...
   { // error (bad length)
      SyslogLogger::log(LOG_WARNING, "%s: received flow control packet length mismatch %d\n",
         __func__, (int)wc.byte_len);
      return -1;
   }
...
int __IBVSocket_waitForRecvCompletionEvent(IBVSocket* _this, int timeoutMS, struct ibv_wc* outWC)
{
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%s:%d: poll CQ failed. result: %d\n",
         __func__, __LINE__, numImmediateEvents);
      return -1;
   }
   else
   ...
   for( ; ; ) /* (loop until "wc retrieved" or "timeout" or "error") */
   {
      ...
      if(unlikely(epollRes < 0) )
      {
         ...
         SyslogLogger::log(LOG_WARNING, "%s: epoll error. SysErr: %d (%s)\n",
      ...
      { // cm event incoming
         ...
         {
            syslog_fhgfs_connerr(stderr, "%s: Ingoring received event (%d: %s)\n",
               __func__, event->event, rdma_event_str(event->event) ); // debug in
      ...
      {
         SyslogLogger::log(LOG_WARNING, "%s: Failed to get cq_event\n", __func__);
         return -1;
      }

      if(unlikely(ev_cq != commContext->recvCQ) )
      {
         SyslogLogger::log(LOG_WARNING, "%s: CQ event for unknown CQ %p\n", __func__, ev_cq);
         return -1;
      }
   ...
   int numSendElements, int numWriteElements, int numReadElements)
{
   ...
   do
   {
      ...
      {
         SyslogLogger::log(LOG_WARNING, "%d:%s: bad ibv_poll_cq result: %d\n", __LINE__, __func__,
            numElements);
      ...
      for(i=0; i < numElements; i++)
      {
         ...
         switch(wc[i].opcode)
         {
            case IBV_WC_SEND:
            {
               ...
               if(unlikely(bufIndex >= commContext->commCfg.bufNum) )
               {
                  SyslogLogger::log(LOG_WARNING, "%d:%s: bad send completion wr_id 0x%x\n",
               ...
               {
                  SyslogLogger::log(LOG_WARNING,
                     "%d:%s: received bad/unexpected send completion\n", __LINE__, __func__);
            ...
            case IBV_WC_RDMA_WRITE:
            {
               ...
               {
                  SyslogLogger::log(LOG_WARNING, "%d:%s: bad write completion wr_id 0x%x\n",
                     __LINE__, __func__, (int)wc[i].wr_id);
               ...
               {
                  SyslogLogger::log(LOG_WARNING,
                     "%d:%s: received bad/unexpected RDMA write completion\n", __LINE__, __func__);
            ...
            case IBV_WC_RDMA_READ:
            {
               ...
               {
                  SyslogLogger::log(LOG_WARNING, "%d:%s: bad read completion wr_id 0x%x\n",
                     __LINE__, __func__, (int)wc[i].wr_id);
               ...
               {
                  SyslogLogger::log(LOG_WARNING,
                     "%d:%s: received bad/unexpected RDMA read completion\n",__LINE__, __func__);
            ...
            default:
            {
               SyslogLogger::log(LOG_WARNING, "%d:%s: bad/unexpected completion opcode %d\n",
                  __LINE__, __func__, wc[i].opcode);
...
int IBVSocket_checkConnection(IBVSocket* _this)
{
   ...
   {
      SyslogLogger::log(LOG_WARNING, "%s: Detected QP error state\n", __func__);
...
bool IBVSocket_checkDelayedEvents(IBVSocket* _this)
{
   ...
   if(_this->delayedCmEventsQ->size() > 0)
      return true;
   ...
   {
      SyslogLogger::log(LOG_WARNING,
         "%d:%s: set conn manager channel non-blocking failed. errno: %d\n",
         __LINE__, __func__, errno);
      return false;
   }
   ...
   {
      SyslogLogger::log(LOG_WARNING,
         "%d:%s: set conn manager channel blocking failed. errno: %d\n", __LINE__, __func__, errno);
      return false;
   }
...
void __IBVSocket_close(IBVSocket* _this)
{
   ...
   { // ack all queued events
      while(_this->delayedCmEventsQ->size() )
      {
...
bool __IBVSocket_initEpollFD(IBVSocket* _this)
{
   ...
   if(_this->epollFD == -1)
   {
      perror("epoll_create");
      SyslogLogger::log(LOG_WARNING,
         "%s:%d: epoll initialization error\n", __func__, __LINE__);
   ...
   if(epollAddRes == -1)
   {
      ...
      SyslogLogger::log(LOG_WARNING,
         "%s:%d: Unable to add sock to epoll set\n", __func__, __LINE__);
