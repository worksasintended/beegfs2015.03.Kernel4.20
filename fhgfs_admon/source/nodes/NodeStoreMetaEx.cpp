...
NodeStoreMetaEx::NodeStoreMetaEx() throw(InvalidConfigException) :
   NodeStoreServers(NODETYPE_Meta, false)
{
   // nothing to be done here
}

bool NodeStoreMetaEx::addOrUpdateNode(MetaNodeEx** node)
{
   ...
   { // node exists already => update
      ...
      MetaNodeEx *oldNode = (MetaNodeEx *) nodeRefer->getReferencedObject();
 ...
 */
bool NodeStoreMetaEx::addOrUpdateNode(Node** node)
{
...
bool NodeStoreMetaEx::addOrUpdateNodeEx(Node** node, uint16_t* outNodeNumID)
{
   std::string nodeID( (*node)->getID() );
   uint16_t nodeNumID = (*node)->getNumID();
   ...
   { // new node
      ...
      MetaNodeEx* metaNode = new MetaNodeEx(
         nodeID, nodeNumID, (*node)->getPortUDP(), (*node)->getPortTCP(), nicList);

      delete(*node);
      *node = metaNode;

      metaNode->initializeDBData();
   }

   bool retVal = addOrUpdateNodeUnlocked(node, NULL);
...
void NodeStoreMetaEx::releaseMetaNode(MetaNodeEx** node)
{
