...
void Database::createInitialTables()
{
   ...
   {
      std::string pw = Crypt::DoMD5(DEFAULT_PASS_INFORMATION);
   ...
   {
      std::string pw = Crypt::DoMD5(DEFAULT_PASS_ADMIN);
      std::string insertQueryStr = "INSERT INTO '" +
         tableNames[TableNames_users] + "' "
         "VALUES('admin','" + pw + "','0');";
      WritingQuery(insertQueryStr);
 ...
 */
void Database::insertMetaNodeData(std::string nodeID, uint16_t nodeNumID, MetaNodeDataContent data)
{
 ...
 * @data the MetaNodeDataContent to write
 */
void Database::insertMetaNodeData(std::string nodeID, uint16_t nodeNumID, TabType tabType,
 ...
 */
void Database::insertStorageNodeData(std::string nodeID, uint16_t nodeNumID,
 ...
 * @data the StorageNodeDataContent to write
 */
void Database::insertStorageNodeData(std::string nodeID, uint16_t nodeNumID, TabType tabType,
   StorageNodeDataContent data)
{
   // choose the right table
   std::string tab;
 ...
 *
 */
void Database::getMetaNodeSets(uint16_t nodeNumID, TabType tabType, long minTime, long maxTime,
 ...
 */
void Database::getStorageNodeSets(uint16_t nodeNumID, TabType tabType, long minTime, long maxTime,
 ...
 */
int Database::addMetaNodeToGroup(std::string nodeID, uint16_t nodeNumID, int groupID)
{
 ...
 */
int Database::addMetaNodeToGroup(std::string nodeID, uint16_t nodeNumID, std::string groupName)
{
   int groupID = getGroupID(groupName);
 ...
 */
int Database::delMetaNodeFromGroup(uint16_t nodeNumID, int groupID)
{
   if (groupID == DEFAULT_GROUP_ID)
   {
      return 0;
   ...
   std::string query = "DELETE FROM '" + tableNames[TableNames_groupmembersMeta] +
      "' WHERE nodeNumID=" + StringTk::uintToStr(nodeNumID) + " AND groupID=" +
      StringTk::intToStr(groupID) + ";";
   try
   {
      WritingQuery(query);
 ...
 * GUI yet)
 */
int Database::delMetaNodeFromGroup(uint16_t nodeNumID, std::string groupName)
{
   int groupID = getGroupID(groupName);
 ...
 */
int Database::addStorageNodeToGroup(std::string nodeID, uint16_t nodeNumID, int groupID)
{
   if (groupID == DEFAULT_GROUP_ID)
   {
      return 0;
 ...
 * are not used in GUI yet)
 */
int Database::addStorageNodeToGroup(std::string nodeID, uint16_t nodeNumID, std::string groupName)
{
   int groupID = getGroupID(groupName);
 ...
 */
int Database::delStorageNodeFromGroup(uint16_t nodeNumID, int groupID)
{
   if (groupID == DEFAULT_GROUP_ID)
   {
      return 0;
 ...
 * GUI yet)
 */
int Database::delStorageNodeFromGroup(uint16_t nodeNumID, std::string groupName)
{
   int groupID = getGroupID(groupName);
...
void Database::getMetaNodesWithoutGroup(StringList *outNodes)
{
   ...
   NodeList metaNodes;
   metaNodeStore->referenceAllNodes(&metaNodes);
   for (NodeListIter nodeIter = metaNodes.begin(); nodeIter != metaNodes.end(); nodeIter++)
   {
   ...
   metaNodeStore->releaseAllNodes(&metaNodes);
}
...
void Database::getStorageNodesWithoutGroup(StringList *outNodes)
{
   ...
   storageNodeStore->referenceAllNodes(&storageNodes);
   for (NodeListIter nodeIter = storageNodes.begin(); nodeIter != storageNodes.end(); nodeIter++)
   {
      std::string nodeID = (*nodeIter)->getID();
      bool grouped = false;
      for (StringListIter iterGrouped = groupedNodes.begin(); iterGrouped != groupedNodes.end();
   ...
   storageNodeStore->releaseAllNodes(&storageNodes);
}
 ...
 */
bool Database::metaNodeIsInGroup(uint16_t nodeNumID, int groupID)
{
   bool retVal = false;
   sqlite3_stmt *stmt;
   ...
   std::string queryStr = std::string("SELECT nodeID FROM '" +
      ...
      StringTk::uintToStr(nodeNumID) + " AND groupID=" + StringTk::intToStr(groupID) + ";");
 ...
 */
bool Database::metaNodeIsInGroup(uint16_t nodeNumID, std::string groupName)
{
 ...
 * @return true if nodeID is in groupID, false otherwise
 */
bool Database::storageNodeIsInGroup(uint16_t nodeNumID, int groupID)
{
   bool retVal = false;
   sqlite3_stmt *stmt;
 ...
 * @return true if nodeID is in groupName, false otherwise
 */
bool Database::storageNodeIsInGroup(uint16_t nodeNumID, std::string groupName)
{
 ...
 */
int Database::metaNodeGetGroup(uint16_t nodeNumID)
{
 ...
 */
int Database::storageNodeGetGroup(uint16_t nodeNumID)
{
   /* read group from DB; if none is found return default group */
   int retVal = DEFAULT_GROUP_ID;
   sqlite3_stmt *stmt;
 ...
 */
void Database::getStorageNodes(UInt16List *outList)
{
   sqlite3_stmt *stmt;
   ...
   while (stepResult == SQLITE_ROW)
   {
      ...
      outList->push_back((uint16_t)res);
      stepResult = sqlite3_step(stmt);
   }
 ...
 * @outList reference to a UInt16List to write the output to
 */
void Database::getMetaNodes(UInt16List *outList)
{
   sqlite3_stmt *stmt;
 ...
 * @outNodes list to save the found nodes into
 */
void Database::getMetaNodesInGroup(int groupID, UInt16List *outNodes)
{
   sqlite3_stmt *stmt;
 ...
 * @outNodes list to save the found nodes into
 */
void Database::getMetaNodesInGroup(std::string groupName, UInt16List *outNodes)
{
   int groupID = getGroupID(groupName);
   return getMetaNodesInGroup(groupID, outNodes);
}
 ...
 * @outNodes list to save the found nodes into
 */
void Database::getStorageNodesInGroup(int groupID, UInt16List *outNodes)
{
   sqlite3_stmt *stmt;
 ...
 * @outNodes list to save the found nodes into
 */
void Database::getStorageNodesInGroup(std::string groupName, UInt16List *outNodes)
{
   int groupID = getGroupID(groupName);
   return getStorageNodesInGroup(groupID, outNodes);
}
