...
class DataRequestorStats : public PThread
{
   public:
      DataRequestorStats(unsigned id, unsigned intervalSecs, unsigned maxLines,
         NodeType nodeType, bool perUser) : PThread("DataReq-Stats-" + StringTk::uintToStr(id)),
         id(id)
      {
         log.setContext("DataReq-Stats");

         this->cfgOptions = CfgClientStatsOptions(intervalSecs, maxLines, nodeType, perUser);
         this->stats = new ClientStats(&this->cfgOptions);
         this->status = DataRequestorStatsStatus_INITIALIZED;

         this->dataSequenceID = 0;
      }
