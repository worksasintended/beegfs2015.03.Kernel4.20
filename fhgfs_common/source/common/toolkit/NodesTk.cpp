...
bool NodesTk::downloadNodes(Node* sourceNode, NodeType nodeType, NodeList* outNodeList,
   bool silenceLog, uint16_t* outRootNumID)
{
   ...
   bool silenceLog)
{
   ...
   respMsgCast->parseBuddyGroupIDs(outBuddyGroupIDs);
   respMsgCast->parsePrimaryTargetIDs(outPrimaryTargetIDs);
   respMsgCast->parseSecondaryTargetIDs(outSecondaryTargetIDs);
   ...
   UInt8List* outReachabilityStates, UInt8List* outConsistencyStates, bool silenceLog)
{
   ...
   if (outReachabilityStates)
      respMsgCast->parseReachabilityStates(outReachabilityStates);

   if (outConsistencyStates)
      respMsgCast->parseConsistencyStates(outConsistencyStates);
 ...
 */
bool NodesTk::downloadStatesAndBuddyGroups(Node* sourceNode, UInt16List* outBuddyGroupIDs,
...
{
   GetStatesAndBuddyGroupsMsg msg(NODETYPE_Storage);
   ...
   respMsgCast->parseTargetIDs(outTargetIDs);
   respMsgCast->parseReachabilityStates(outTargetReachabilityStates);
   respMsgCast->parseConsistencyStates(outTargetConsistencyStates);
