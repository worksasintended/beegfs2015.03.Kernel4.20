 ...
 */
bool StorageTk::createPathOnDisk(Path& path, bool excludeLastElement, mode_t* inMode)
{
...
bool StorageTk::createPathOnDisk(int fd, Path& relativePath, bool excludeLastElement,
   mode_t* inMode)
{
   const StringList* pathElems = relativePath.getPathElems();
   std::string pathStr = relativePath.isAbsolute() ? "/" : "";
   unsigned pathCreateDepth = excludeLastElement ? pathElems->size() -1 : pathElems->size();
      ...
      int64_t* outInodesFree)
{
   ...
   bool spaceFilePathExists = StorageTk::pathExists(spaceFilePath.getPathAsStr() );
   if(spaceFilePathExists)
   {
      ICommonConfig::loadStringListFile(spaceFilePath.getPathAsStr().c_str(), spaceList);
   ...
   bool inodesFilePathExists = StorageTk::pathExists(inodesFilePath.getPathAsStr() );
   if(inodesFilePathExists)
   {
      ICommonConfig::loadStringListFile(inodesFilePath.getPathAsStr().c_str(), inodesList);
...
void StorageTk::checkAndUpdateStorageFormatFile(const std::string pathStr, int minVersion,
   int currentVersion, StringMap* outFormatProperties) throw(InvalidConfigException)
{
   Path dirPath(pathStr);
   Path filePath(dirPath, STORAGETK_FORMAT_FILENAME);
   std::string filePathStr = filePath.getPathAsStr();
   ...
   int formatFileVersion = StringTk::strToInt(iter->second.c_str() );
   if(formatFileVersion < minVersion || formatFileVersion > currentVersion)
   {
      throw InvalidConfigException(
         std::string("Incompatible storage format: ")                         +
         iter->second + " (required min: " +  StringTk::intToStr(minVersion)  +
         " required max : " + StringTk::intToStr(currentVersion) + ")");
   }

   if (formatFileVersion < currentVersion)
   {  // update the version
      Path dirPath(pathStr);
      Path filePath(dirPath, STORAGETK_FORMAT_FILENAME);
      std::string filePathStr = filePath.getPathAsStr();
 ...
 */
bool StorageTk::checkStorageSessionsFileExists(const std::string& pathStr)
{
   Path dirPath(pathStr);
...
/**
 * Get path to chunk file and additional PathVec to chunk file (not including file name), both
 ...
 * @param outChunkDirPathVec path to parent dir of chunk file (filled-in for >V2 only).
...
void StorageTk::getChunkDirChunkFilePath(const PathInfo* pathInfo, std::string entryID,
   bool hasOrigFeature, PathVec& outChunkDirPathVec, std::string& outChunkFilePathStr)
{
   if (!hasOrigFeature )
   {  // old  2012.10 style storage layout
      outChunkFilePathStr = StorageTk::getFileChunkPathV2(entryID, &outChunkDirPathVec);
   }
   else
   { // new 2014.01 style storage layout
      StorageTk::getChunkDirV3PathVec(pathInfo, outChunkDirPathVec);
      outChunkFilePathStr = outChunkDirPathVec.getPathAsStr() + '/' + entryID;
   }
}
...
bool StorageTk::rmDirContentsRec(int fd, std::string relativeDirPath)
{
   ...
   int dirFD = openat(fd, relativeDirPath.c_str(), O_RDONLY);

   if ( dirFD == -1 )
   {
      LogContext(__func__).logErr(
         "Unable to open directory fd. relativeDirPath: " + relativeDirPath + "; error: "
            + System::getErrString());
   ...
   {
      LogContext(__func__).logErr(
         "Unable to create directory handle. relativeDirPath: " + relativeDirPath + "; error: "
            + System::getErrString());
   ...
   for ( ;; )
   {
      ...
      else
      {
         ...
         std::string subPath = relativeDirPath + "/" + std::string(dirEntry->d_name);

         bool isDirectory = false;

         struct stat statBuf;
         int statRes = stat(subPath.c_str(), &statBuf);
         ...
         if (isDirectory)
         {
            ...
            bool res = rmDirContentsRec(fd, subPath);
            if ( !res )
            {
               retVal = false;
               break;
            ...
            int unlinkRes = unlinkat(fd, subPath.c_str(), AT_REMOVEDIR);
            if ( ( unlinkRes == -1 ) && (errno != ENOENT) )
            {
               LogContext(__func__).logErr(
                  "Unable to unlink directory. relativePath: " + subPath + "; errorStr: "
                     + System::getErrString());
         ...
         {
            int unlinkRes = unlinkat(fd, subPath.c_str(), 0);
            if ( (unlinkRes == -1 ) && (errno != ENOENT) )
            {
               LogContext(__func__).logErr(
                  "Unable to unlink file. relativePath: " + subPath + "; errorStr: "
                     + System::getErrString());
