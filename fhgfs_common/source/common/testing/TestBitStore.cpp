...
void TestBitStore::checkSerialization(BitStore* store)
{
   ...
   unsigned serialLen = store->serialLen();

   char* buf = (char*) malloc(serialLen);

   unsigned serializationRes = store->serialize(buf);

   CPPUNIT_ASSERT_MESSAGE("BitStore serialLen and serializationRes don't match.",
      serialLen == serializationRes);

   unsigned preprocessDeserLen;
   const char* deserStoreStart;

   bool preprocessSuccess = secondStore.deserializePreprocess(buf, serialLen, &deserStoreStart,
      &preprocessDeserLen);

   CPPUNIT_ASSERT_MESSAGE("BitStore deserialization precrocessing failed.", preprocessSuccess);

   CPPUNIT_ASSERT_MESSAGE("Bad BitStore deserialization length.",
      preprocessDeserLen == serialLen);

   unsigned deserLen;
   secondStore.deserialize(deserStoreStart, &deserLen);

   CPPUNIT_ASSERT_MESSAGE("BitStores preprocess deserialization length differs from "
      "deserialization length.", deserLen == preprocessDeserLen);

   CPPUNIT_ASSERT_MESSAGE("BitStores are not equals after deserialization.",
      store->compare(&secondStore) );
