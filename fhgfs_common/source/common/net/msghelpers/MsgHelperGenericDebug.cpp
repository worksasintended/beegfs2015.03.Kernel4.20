   ...
   std::string headline)
{
   ...
   while(node)
   {
      ...
      node = nodes->referenceNextNodeAndReleaseOld(node);
   }
   ...
   TargetStateStore* targetStateStore)
{
   ...
   UInt16ListConstIter idIter = targetIDs.begin();
   UInt8ListConstIter reachabilityStateIter = targetReachabilityStates.begin();
   UInt8ListConstIter consistencyStateIter = targetConsistencyStates.begin();

   for( ; idIter != targetIDs.end(); idIter++, reachabilityStateIter++, consistencyStateIter++)
   {
      returnStream << *idIter << " "
            << TargetStateStore::stateToStr( (TargetReachabilityState)*reachabilityStateIter) << " "
            << TargetStateStore::stateToStr( (TargetConsistencyState)*consistencyStateIter)
            << std::endl;
   }
