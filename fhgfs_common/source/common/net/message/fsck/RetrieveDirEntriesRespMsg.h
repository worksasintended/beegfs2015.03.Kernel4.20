...
class RetrieveDirEntriesRespMsg : public NetMessage
{
   ...
   public:
      ...
      {
         RetrieveDirEntriesRespMsg* msgIn = (RetrieveDirEntriesRespMsg*) msg;

         FsckContDirList contDirsIn;
         msgIn->parseContDirs(&contDirsIn);

         FsckDirEntryList dirEntriesIn;
         msgIn->parseDirEntries(&dirEntriesIn);

         FsckFileInodeList fileInodesIn;
         msgIn->parseInlinedFileInodes(&fileInodesIn);

         if (! ListTk::listsEqual(this->fsckContDirs, &contDirsIn) )
            return TestingEqualsRes_FALSE;

         if (! ListTk::listsEqual(this->fsckDirEntries, &dirEntriesIn) )
            return TestingEqualsRes_FALSE;

         if (! ListTk::listsEqual(this->inlinedFileInodes, &fileInodesIn) )
            return TestingEqualsRes_FALSE;
