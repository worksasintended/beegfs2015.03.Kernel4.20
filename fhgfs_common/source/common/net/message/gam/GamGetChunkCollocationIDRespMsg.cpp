#include "GamGetChunkCollocationIDRespMsg.h"

bool GamGetChunkCollocationIDRespMsg::deserializePayload(const char* buf, size_t bufLen)
{
   size_t bufPos = 0;
...
void GamGetChunkCollocationIDRespMsg::serializePayload(char* buf)
{
   size_t bufPos = 0;
...
TestingEqualsRes GamGetChunkCollocationIDRespMsg::testingEquals(NetMessage *msg)
{
   if ( msg->getMsgType() == this->getMsgType() )
   {
      GamGetChunkCollocationIDRespMsg* msgCast = (GamGetChunkCollocationIDRespMsg*) msg;
