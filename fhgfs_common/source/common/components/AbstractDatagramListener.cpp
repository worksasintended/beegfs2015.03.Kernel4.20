   ...
   AcknowledgeableMsg* msg, int ackWaitSleepMS, int numRetries)
{
   ...
   bool sendRes = sendToNodesUDPwithAck(&referencedNodes, msg, ackWaitSleepMS, numRetries);
   ...
   int ackWaitSleepMS, int numRetries)
{
   ...
   bool sendRes = sendToNodesUDPwithAck(&nodeList, msg, ackWaitSleepMS, numRetries);
