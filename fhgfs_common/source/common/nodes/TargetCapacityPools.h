...
typedef std::map<uint16_t, UInt16Set> GroupedTargets; // keys: nodeIDs, values: node's targetIDs
...
class TargetCapacityPools
{
   public:
      ...
      bool addOrUpdate(uint16_t targetID, uint16_t nodeID, CapacityPoolType poolType);
      bool addIfNotExists(uint16_t targetID, uint16_t nodeID, CapacityPoolType poolType);
   ...
   private:
      ...
      bool addOrUpdateUnlocked(uint16_t targetID, uint16_t nodeID, CapacityPoolType poolType);
      void removeFromOthersUnlocked(uint16_t targetID, uint16_t previousNodeID,
      ...
      void chooseTargetsInterdomainNoPref(GroupedTargets& groupedTargets, unsigned numTargets,
         UInt16Vector& outTargets, UInt16Vector& outNodes);
      ...
      static uint16_t getNodeIDFromTargetID(uint16_t targetID, const TargetMap& targetMap);
