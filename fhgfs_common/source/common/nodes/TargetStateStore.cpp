   ...
   const UInt16List& primaryTargets, const UInt16List& secondaryTargets)
{
   ...
   UInt8ListConstIter reachabilityStatesIter = reachabilityStates.begin();
   UInt8ListConstIter consistencyStatesIter = consistencyStates.begin();

   for( ; targetIDsIter != targetIDs.end();
      targetIDsIter++, reachabilityStatesIter++, consistencyStatesIter++)
   {
      TargetStateInfo stateInfo(
         static_cast<TargetReachabilityState>(*reachabilityStatesIter),
         static_cast<TargetConsistencyState>(*consistencyStatesIter) );
      statesMapNewTmp[*targetIDsIter] = stateInfo;
   }
   ...
   for( ; keysIter != buddyGroupIDs.end(); keysIter++, primaryIter++, secondaryIter++)
      newGroups[*keysIter] = MirrorBuddyGroup(*primaryIter, *secondaryIter);
