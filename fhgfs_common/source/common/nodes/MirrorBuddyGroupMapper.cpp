...
FhgfsOpsErr MirrorBuddyGroupMapper::mapMirrorBuddyGroup(uint16_t buddyGroupID,
   uint16_t primaryTargetID, uint16_t secondaryTargetID, bool allowUpdate = true,
   uint16_t* outNewBuddyGroup = NULL)
{
...
/**
 * @return true if the buddyGroupID was mapped
 */
bool MirrorBuddyGroupMapper::unmapMirrorBuddyGroup(uint16_t buddyGroupID)
{
   SafeRWLock safeLock(&rwlock, SafeRWLock_WRITE); // L O C K
 ...
 */
void MirrorBuddyGroupMapper::syncGroupsFromLists(UInt16List& buddyGroupIDs,
   UInt16List& primaryTargets, UInt16List& secondaryTargets)
{
   ...
   syncGroupsFromLists(buddyGroupIDs, mbgList);
}
...
uint16_t MirrorBuddyGroupMapper::generateID()
{
   ...
   return MirrorBuddyGroupCreator::generateID(this->targetMapper, &groupIDs);
}
