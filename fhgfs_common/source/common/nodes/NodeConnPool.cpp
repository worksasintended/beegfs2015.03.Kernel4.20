 ...
 */
NodeConnPool::NodeConnPool(Node* parentNode, unsigned short streamPort, NicAddressList& nicList)
{
...
void NodeConnPool::authenticateChannel(Socket* sock)
{
   ...
   char* msgBuf = MessagingTk::createMsgBuf(&authMsg);

   try
   {
      sock->sendto(msgBuf, authMsg.getMsgLength(), 0, NULL, 0);
   }
   catch(SocketException& e)
   {
      free(msgBuf);
