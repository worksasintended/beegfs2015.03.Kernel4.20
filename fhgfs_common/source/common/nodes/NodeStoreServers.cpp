...
#define NODESTORE_MAX_NODENUMIDS (USHRT_MAX-1) /* maximum nuber of IDs, -1 for reserved value "0" */
#define NODESTORE_MAX_NODENUMID  (USHRT_MAX)   /* highest valid ID value */
...
{
   return addOrUpdateNodeEx(node, NULL);
}
 ...
 */
bool NodeStoreServers::addOrUpdateNodeEx(Node** node, uint16_t* outNodeNumID)
{
...
bool NodeStoreServers::addOrUpdateNodeUnlocked(Node** node, uint16_t* outNodeNumID)
{
   ...
   { // node was in the active store already => update it
      ...
      { // update node
         ...
         nodeNonRef->updateFeatureFlagsThreadSafe( (*node)->getNodeFeatures() );
      }
   ...
   { // node wasn't in active store yet
      ...
      uint16_t existingNumID = retrieveNumIDFromStringID( (*node)->getID() );
 ...
 */
Node* NodeStoreServers::referenceNodeByStringID(std::string nodeStrID)
{
   Node* node = referenceFirstNode();
   while(node)
   {
      if(node->getID() == nodeStrID)
         return node;

      node = referenceNextNodeAndReleaseOld(node);
   }
...
bool NodeStoreServers::deleteNode(uint16_t id)
{
   ...
   { // node exists in active store
      ...
      if(stateStore)
         stateStore->removeTarget(id); // remove node from target states
   }
 ...
 */
std::string NodeStoreServers::getTypedNodeID(uint16_t numID)
{
   SafeMutexLock mutexLock(&mutex); // L O C K
   ...
   { // node exists in active store
      ...
      nodeID = node->getTypedNodeID();
   }
   else
   if(localNode && (numID == localNode->getNumID() ) )
      nodeID = localNode->getTypedNodeID();
   else
      nodeID = std::string("Unknown node: ") +
         Node::nodeTypeToStr(storeType) + " [ID: " + StringTk::uintToStr(numID) + "]";
