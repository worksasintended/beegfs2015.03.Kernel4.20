...
size_t FsckDirEntry::serialize(char* outBuf)
{
   ...
   // hasInlinedInode
   bufPos += Serialization::serializeBool(&outBuf[bufPos], this->getHasInlinedInode());
...
bool FsckDirEntry::deserialize(const char* buf, size_t bufLen, unsigned* outLen)
{
   ...
   {
      // hasInlinedInode
      unsigned hasInlinedInodeBufLen;

      if(!Serialization::deserializeBool(&buf[bufPos], bufLen-bufPos,
         &hasInlinedInode, &hasInlinedInodeBufLen) )
         return false;

      bufPos += hasInlinedInodeBufLen;
   }
   ...
   this->update(id, name, parentID, entryOwnerNodeID, inodeOwnerNodeID,
      (FsckDirEntryType)intEntryType, hasInlinedInode, saveNodeID, saveDevice, saveInode);
...
unsigned FsckDirEntry::serialLen(void)
{
   unsigned length =
      ...
      Serialization::serialLenBool() + //hasInlinedInode
