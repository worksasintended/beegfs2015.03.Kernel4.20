...
void AbstractConfig::loadDefaults(bool addDashes)
{
   ...
   configMapRedefine("sysUpdateTargetStatesSecs",  "30", addDashes);

   configMapRedefine("runUnitTests",             "false", addDashes);
   configMapRedefine("testOutputFile",           "", addDashes);
   configMapRedefine("testOutputFormat",         "", addDashes);
}
   ...
   throw(InvalidConfigException)
{
   for(StringMapIter iter = configMap.begin(); iter != configMap.end(); )
   {
      ...
      if(testConfigMapKeyMatch(iter, "logLevel", addDashes) )
         logLevel = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "logErrsToStdlog", addDashes) )
         logErrsToStdlog = StringTk::strToBool(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "logNoDate", addDashes) )
         logNoDate = StringTk::strToBool(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "logStdFile", addDashes) )
         logStdFile = iter->second;
      else
      if(testConfigMapKeyMatch(iter, "logErrFile", addDashes) )
         logErrFile = iter->second;
      else
      if(testConfigMapKeyMatch(iter, "logNumLines", addDashes) )
         logNumLines = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "logNumRotatedFiles", addDashes) )
         logNumRotatedFiles = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connPortShift", addDashes) )
         connPortShift = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connClientPortUDP", addDashes) )
         connClientPortUDP = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connStoragePortUDP", addDashes) )
         connStoragePortUDP = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connMetaPortUDP", addDashes) )
         connMetaPortUDP = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connAdmonPortUDP", addDashes) )
         connAdmonPortUDP = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connMgmtdPortUDP", addDashes) )
         connMgmtdPortUDP = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connStoragePortTCP", addDashes) )
         connStoragePortTCP = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connMetaPortTCP", addDashes) )
         connMetaPortTCP = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connHelperdPortTCP", addDashes) )
         connHelperdPortTCP = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connMgmtdPortTCP", addDashes) )
         connMgmtdPortTCP = StringTk::strToInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connUseSDP", addDashes) )
         connUseSDP = StringTk::strToBool(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connUseRDMA", addDashes) )
         connUseRDMA = StringTk::strToBool(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connBacklogTCP", addDashes) )
         connBacklogTCP = StringTk::strToUInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connMaxInternodeNum", addDashes) )
         connMaxInternodeNum = StringTk::strToUInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connNonPrimaryExpiration", addDashes) )
      {
         // superseded by connFallbackExpirationSecs, ignored here for config file compatibility
      }
      else
      if(testConfigMapKeyMatch(iter, "connFallbackExpirationSecs", addDashes) )
         connFallbackExpirationSecs = StringTk::strToUInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connRDMABufSize", addDashes) )
         connRDMABufSize = StringTk::strToUInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connRDMABufNum", addDashes) )
         connRDMABufNum = StringTk::strToUInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connRDMATypeOfService", addDashes) )
         connRDMATypeOfService = (uint8_t)StringTk::strToUInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "connNetFilterFile", addDashes) )
         connNetFilterFile = iter->second;
      else
      if(testConfigMapKeyMatch(iter, "connAuthFile", addDashes) )
      ...
      if(testConfigMapKeyMatch(iter, "sysMgmtdHost", addDashes) )
         sysMgmtdHost = iter->second;
      else
      if(testConfigMapKeyMatch(iter, "sysUpdateTargetStatesSecs", addDashes) )
         sysUpdateTargetStatesSecs = StringTk::strToUInt(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "runUnitTests", addDashes) )
         runUnitTests = StringTk::strToBool(iter->second);
      else
      if(testConfigMapKeyMatch(iter, "testOutputFile", addDashes) )
         testOutputFile = iter->second;
      else
      if(testConfigMapKeyMatch(iter, "testOutputFormat", addDashes) )
         testOutputFormat = iter->second;
      else
 ...
 */
void AbstractConfig::initConnAuthHash(std::string connAuthFile,
   uint64_t* outConnAuthHash) throw(InvalidConfigException)
{
   ...
   ssize_t readRes = read(fd, buf, ABSTRACTCONF_AUTHFILE_READSIZE);
   ...
   uint64_t high = BufferTk::hash32(buf, len1stHalf);
   uint64_t low  = BufferTk::hash32(&buf[len1stHalf], len2ndHalf);
