...
int ModeStorageBench::execute()
{
   ...
   for (UInt16ListIter iter = nodeIDs.begin(); iter != nodeIDs.end(); iter++)
   {
...
int ModeStorageBench::checkConfig(Node* mgmtNode, NodeStoreServers* storageNodes, StringMap* cfg)
{
   ...
   if( (iter != cfg->end()) && (cfgTargetIDs.getSize() == 0) && (!iter->second.empty()) )
   {
      ...
      UInt16ListIter nodeIdIter = mappedNodeIDs.begin();
   ...
   if( (iter != cfg->end()) && (cfgTargetIDs.getSize() == 0) && (!iter->second.empty()) )
   {
      ...
      for(StringListIter storageIter = this->cfgStorageServer.begin();
          ...
          storageIter++ )
      {
         if(!storageNodes->isNodeActive(StringTk::strToUInt(*storageIter) ) )
         {
      ...
      for(UInt16ListIter nodeIdIter = mappedNodeIDs.begin(); nodeIdIter != mappedNodeIDs.end(); )
      {
         bool targetFound = false;
         ...
         for(StringListIter inputNodeIter = this->cfgStorageServer.begin();
            ...
            inputNodeIter++)
         {
            if(StringTk::strToUInt(*inputNodeIter) == *nodeIdIter)
            {
               targetFound = true;
               break;
            }
...
void ModeStorageBench::printShortResults(StorageBenchResponseInfoList* responses)
{
   ...
   uint16_t fastestNodeID;
   uint16_t slowestNodeID;
 ...
 *
 */
bool ModeStorageBench::sendCmdAndCollectResult(uint16_t nodeID,
   StorageBenchResponseInfo &response)
{
   bool retVal = false;
