...
int ModeMigrate::doExecute()
{
   ...
   retVal = addNodesToNodeStore(app, mgmtNodes, &mgmtNode);
   if (retVal != APPCODE_NO_ERROR)
      goto out;
 ...
 */
int ModeMigrate::getDestTargets(UInt16List& targetIDs, UInt16List& nodeIDs)
{
   UInt16ListIter targetIter = targetIDs.begin();
   UInt16ListIter nodeIter   = nodeIDs.begin();
   ...
   while (nodeIter != nodeIDs.end() )
   {
      ...
      {
         if (*searchTargetIter == *targetIter)
         {
            isSearchTarget = true;
            break;
         }
      ...
      if (!isSearchTarget)
      {
         NodeTargetMapIter mapIter = targetMap.find(*nodeIter);
         // add target to per node target maps
         if ( mapIter == targetMap.end() )
         {
            UInt16List* newStringList = new UInt16List;
            targetMap.insert(NodeTargetMap::value_type(*nodeIter, newStringList));
            mapIter = targetMap.find(*nodeIter);
         }

         UInt16List* perNodeTargetList = mapIter->second;
         perNodeTargetList->push_back(*targetIter);
 ...
 * @return APPCODE_...
 */
int ModeMigrate::getFromTargets(UInt16List& targetIDs, UInt16List& nodeIDs)
{
   if (this->cfgTargetID)
   ...
   { // get all targetIDs from the given node
      ...
      while (nodeIter != nodeIDs.end() )
      {
         if (*nodeIter == cfgNodeID)
            this->searchStorageTargetIDs.push_back(*targetIter);
   ...
   UInt16List& buddyGroupPrimaryTargetIDs, UInt16List& buddyGroupSecondaryTargetIDs)
{
   ...
   UInt16ListIter buddyGrouptIter = mirrorBuddyGroupIDs.begin();
   UInt16ListIter primaryTargetIter = buddyGroupPrimaryTargetIDs.begin();
   UInt16ListIter secondaryTargetIter = buddyGroupSecondaryTargetIDs.begin();

   for(; buddyGrouptIter != mirrorBuddyGroupIDs.end(),
      primaryTargetIter != buddyGroupPrimaryTargetIDs.end(),
      secondaryTargetIter != buddyGroupSecondaryTargetIDs.end();
      buddyGrouptIter++, primaryTargetIter++, secondaryTargetIter++)
   {
      ...
      while (searchTargetIter != this->searchStorageTargetIDs.end() )
      {
         if( (*primaryTargetIter == *searchTargetIter) ||
            (*secondaryTargetIter == *searchTargetIter) )
         {
            this->searchMirrorBuddyGroupIDs.push_back(*buddyGrouptIter);
            targetFound = true;
            break;
         }
      ...
      if(!targetFound && !mirrorBuddyGroupIDs.empty() )
         destGroupIDs.push_back(*buddyGrouptIter);
   }
 ...
 * @return APPCODE_...
 */
int ModeMigrate::addNodesToNodeStore(App* app, NodeStoreServers* mgmtNodes, Node** outMgmtNode)
{
   ...
   *outMgmtNode = mgmtNode;
   ...
   // for 'find' we always need the meta nodes
   NodeList metaNodes;
   if(!NodesTk::downloadNodes(mgmtNode, NODETYPE_Meta, &metaNodes, false, &rootNodeID) )
   {
      std::cerr << "Download of metadata nodes failed." << std::endl;
      return APPCODE_RUNTIME_ERROR;
   }

   this->nodeStoreMeta = app->getMetaNodes();

   NodesTk::applyLocalNicCapsToList(app->getLocalNode(), &metaNodes);
   NodesTk::moveNodesFromListToStore(&metaNodes, this->nodeStoreMeta);

   this->nodeStoreMeta->setRootNodeNumID(rootNodeID, false);
...
int ModeMigrate::getParams()
{
   ...
   if (iter != cfg->end() )
   {
      ...
      this->cfgNodeID = StringTk::strToUInt(iter->second);
      cfg->erase(iter);
   }
   ...
   if ( (this->cfgNodeType == NODETYPE_Meta) && !this->cfgNodeID)
      this->cfgNodeID = this->cfgTargetID;
...
void ModeMigrate::printHelpFind()
{
   ...
   std::cout << "                         (Note: Metadata mirror nodes are generally not included" << std::endl;
   std::cout << "                         in file search.)" << std::endl;
   ...
   bool* outIsBuddyMirrored)
{
   ...
   bool entryRes = getEntryTargets(path, &stripePattern, &metaOwnerNodeID);
   if (!entryRes)
   {
   ...
   if (stripePattern->getPatternType() == STRIPEPATTERN_BuddyMirror)
      *outIsBuddyMirrored = true;
   else
      *outIsBuddyMirrored = false;

   /* note: mirror mds are currently not included in file search, because mirroring is a directory
...
EntryInfo* ModeMigrate::getEntryInfo(std::string& path)
{
   ...
   if (metaOwnerNode)
      Program::getApp()->getMetaNodes()->releaseNode(&metaOwnerNode);
   ...
   uint16_t* outMetaOwnerNodeID)
{
   ...
   int res = pathToEntryInfo(path, &entryInfo, &metaOwnerNode);
   if (res != APPCODE_NO_ERROR)
      return false;
   ...
   {
      std::cerr << "Received an invalid stripe pattern (" << path << ") " << std::endl;
      retVal = false;
      goto err_cleanup;
   }

   *outStripePattern = pattern;
   ...
   Node** outMetaOwnerNode)
{
   ...
   FhgfsOpsErr findRes =
      MetadataTk::referenceOwner(&path, false, metaNodes, outMetaOwnerNode, outEntryInfo);
...
bool ModeMigrate::checkFileStripes(StripePattern* stripePattern)
{
   ...
   UInt16Vector fileTargetIDs;
   stripePattern->getStripeTargetIDs(&fileTargetIDs);
   ...
   UInt16VectorIter fileTargetIter = fileTargetIDs.begin();
