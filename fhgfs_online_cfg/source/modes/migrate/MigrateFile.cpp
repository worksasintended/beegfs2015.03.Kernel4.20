 ...
 */
bool MigrateFile::migrateRegularFile(void)
{
...
{
   // TODO: Migrate-lock the file on the meta server (file must not be open)
...
int MigrateFile::runCreateIoctl(struct stat* fromStat, const char* symlinkTo)
{
   ...
   struct BeegfsIoctl_MkFile_Arg ioctlData;
   ::memset(&ioctlData, 0, sizeof(ioctlData) );

   int retVal = -1;

   ioctlData.ownerNodeID            = this->parentInfo.getOwnerNodeID();
...
bool MigrateFile::copyData(int fromFD, int tmpFD, struct stat* fromStatData)
{
   ...
   while (readSizeLeft) // read loop
   {
      ...
      // TODO: Make a 4K loop to compare only 4K blocks (and then alignment is important)
