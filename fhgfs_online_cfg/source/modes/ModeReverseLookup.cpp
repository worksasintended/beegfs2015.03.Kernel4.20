...
int ModeReverseLookup::execute()
{
   ...
   NodesTk::deleteListNodes(&metaNodes);

   cleanup_mgmt: mgmtNodes->releaseNode(&mgmtNode);
...
bool ModeReverseLookup::lookupId(std::string id, NodeList *metaNodes, std::string *outName,
   std::string *outParentID)
{
   ...
   *outParentID = linkParentPath;
   ...
   NodeListIter nodeIter = metaNodes->begin();
   Node *node = NULL;
   while ( nodeIter != metaNodes->end() )
   {
      Node *tmpNode = *nodeIter;
      if (tmpNode->getID().compare(linkOwnerNodeID) == 0)
      {
         node = *nodeIter;
         break;
      }
...
bool ModeReverseLookup::getLinkOwnerInfo(std::string id, NodeList *metaNodes,
   std::string *outLinkOwnerID, std::string *outParentID)
{
   NodeListIter nodeIter = metaNodes->begin();
   while (nodeIter != metaNodes->end())
   {
