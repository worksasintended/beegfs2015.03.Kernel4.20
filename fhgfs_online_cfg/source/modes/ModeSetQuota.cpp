...
int ModeSetQuota::execute()
{
   ...
   { // read all quota limits from file and add to limits list
      ...
      if(readFileRetVal >= MODESETQUOTA_ERR_FAILD_LINES)
         ...
         << " are not valid." << mgmtHost << std::endl;
...
void ModeSetQuota::printHelp()
{
   ...
   std::cout << std::endl;
   std::cout << " Example: Set quota size limit to 50 MiB and 50 chunk files for user ID 1000." << std::endl;
   std::cout << "  $ beegfs-ctl --setquota --sizelimit=50M --inodelimit=50 --uid 1000" << std::endl;
   std::cout << std::endl;
   std::cout << " Example: Set group quota limits given in file /tmp/quota_limits.txt." << std::endl;
   std::cout << "  $ beegfs-ctl --setquota --gid --file=/tmp/quota_limits.txt" << std::endl;
   std::cout << std::endl;
   std::cout << " Example: Set quota size limit to 20 MiB and 500 chunk files for user IDs 100" << std::endl;
   std::cout << "          and 110." << std::endl;
   std::cout << "  $ beegfs-ctl --setquota --uid --sizelimit=20M --inodelimit=500 --list 100,110" << std::endl;
   std::cout << std::endl;
...
bool ModeSetQuota::uploadQuotaLimitsAndCollectResponses(Node* mgmtNode)
{
   ...
   for(int messageNumber = 0; messageNumber < maxMessageCount; messageNumber++)
   {
      ...
      if (!respMsgCast->getValue() )
      {
         ...
         limitCounter = this->quotaLimitsList.size();
