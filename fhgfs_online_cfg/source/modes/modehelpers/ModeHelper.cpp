...
bool ModeHelper::registerNode()
{
   ...
   HeartbeatMsg msg(localNodeID.c_str(), 0, NODETYPE_Client, &nicList, nodeFeatureFlags);
...
bool ModeHelper::unregisterNode()
{
   ...
   RemoveNodeMsg msg(localNodeID.c_str(), localNodeNumID, NODETYPE_Client);
...
FhgfsOpsErr ModeHelper::getBuddyResyncStats(uint16_t targetID,
   BuddyResyncJobStats& outStats)
{
   ...
   char* respBuf = NULL;
   NetMessage* respMsg = NULL;
   GetStorageResyncStatsRespMsg* respMsgCast;

   GetStorageResyncStatsMsg msg(targetID);

   // request/response
   commRes = MessagingTk::requestResponse(
      storageNode, &msg, NETMSGTYPE_GetStorageResyncStatsResp, &respBuf, &respMsg);
   if(!commRes)
   {
   ...
   respMsgCast = (GetStorageResyncStatsRespMsg*)respMsg;
